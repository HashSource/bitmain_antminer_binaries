...
void __cdecl mining_parse_job(char *str, uint8_t *target, int target_len, char *job_id, int job_id_len, uint8_t *header, int header_len, int *fixed_nonce_bytes);
...
void __cdecl write_lcd_no_memset(unsigned int line, char *buf, unsigned int size);
...
char byte_36C3C[4] = { '\0', '\0', '\0', '\0' }; // weak
...
_UNKNOWN *_frame_dummy_init_array_entry = (_UNKNOWN *)0x1132D; // weak
_UNKNOWN *_do_global_dtors_aux_fini_array_entry = (_UNKNOWN *)0x11315; // weak
...
char nibble[16] = "0123456789abcdef"; // idb
  ...
  if ( read_back_data[1] == 23 && read_back_data[0] == 5 )
  {
    ...
    if ( HIBYTE(crc) == read_back_data[3] && (unsigned __int8)crc == read_back_data[4] )
    {
      *version = read_back_data[2];
      if ( use_syslog || opt_log_output || opt_log_level > 1 )
      {
        snprintf(tmp42, 0x400u, "--- %s ok, version = 0x%02x\n\n", "get_PIC16F1704_software_version", *version);
        applog(2, tmp42, 0);
      }
      return 1;
    }
    else
    {
      if ( use_syslog || opt_log_output || opt_log_level > 1 )
      {
        snprintf(tmp42, 0x400u, "--- %s failed! crc = 0x%04x\n\n", "get_PIC16F1704_software_version", crc);
        applog(2, tmp42, 0);
      }
      return 0;
    }
...
void __cdecl my_log_curses(int prio, const char *datetime, const char *str, bool force)
{
  if ( !opt_quiet || !prio )
  {
    ...
    {
      if ( prio == 1 )
        fprintf((FILE *)_bss_start, "\x1B[1;34m%s%s\x1B[0m", datetime, str);
      else
        fprintf((FILE *)_bss_start, "%s%s", datetime, str);
    }
    else
    {
      fprintf((FILE *)_bss_start, "\x1B[0;31m%s%s\x1B[0m", datetime, str);
    }
...
// 47788: using guessed type _UNKNOWN *_frame_dummy_init_array_entry;
// 4778C: using guessed type _UNKNOWN *_do_global_dtors_aux_fini_array_entry;
