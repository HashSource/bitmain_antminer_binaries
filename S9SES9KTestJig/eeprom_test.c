...
_UNKNOWN *_frame_dummy_init_array_entry = (_UNKNOWN *)0x10A25; // weak
_UNKNOWN *_do_global_dtors_aux_fini_array_entry = (_UNKNOWN *)0x10A0D; // weak
...
{
  _BOOL4 v4; // r4
  uint8_t buf[259]; // [sp+10h] [bp+10h] BYREF
...
int __fastcall main(int argc, const char **argv, const char **envp)
{
  ...
  char path[256]; // [sp+110h] [bp+108h] BYREF
...
bool __cdecl eeprom_init(char *path)
{
  ...
  else
  {
    ...
    {
      if ( chain_list[i_0] )
        write_eeprom_store_into_at24c02((eeprom_store_t *)vars0[i_0 - 21], i_0);
    }
...
void __cdecl eeprom_dump(uint8_t chain)
{
  ...
  uint8_t buf[8]; // [sp+10h] [bp+10h] BYREF
...
bool __cdecl conf_process(int chain, uint16_t (*freq_data)[9], const char *str)
{
  int v4; // r4
  int v5; // r0
  char chain_stamp[16]; // [sp+14h] [bp+14h] BYREF
  char temp_index[16]; // [sp+24h] [bp+24h] BYREF
  char temp_freq[16]; // [sp+34h] [bp+34h] BYREF
  ...
  if ( strstr(str, chain_stamp) )
  {
    ...
    for ( i_1 = 0; i_1 < stamp_num; ++i_1 )
    {
      ...
      strncpy(temp_freq, &str[stamp_flag[i_1] + 1], line_flag[i_1 + 1] - stamp_flag[i_1] - 1);
      temp_freq[line_flag[i_1 + 1] - stamp_flag[i_1]] = 0;
      memset(temp_index, 0, sizeof(temp_index));
      strncpy(temp_index, &str[line_flag[i_1] + 1], stamp_flag[i_1] - line_flag[i_1] - 1);
...
bool __cdecl scan_eeprom_config_file(const char *conf_file_path, uint16_t (*freq_data)[12][9])
{
  ...
  _DWORD vars0[2]; // [sp+4C0h] [bp+4C0h] BYREF
  ...
  if ( fp )
  {
    ...
    if ( file_buf )
    {
      ...
      for ( i_0 = 0; i_0 <= 15; ++i_0 )
      {
        vars0[i_0 - 45] = malloc(0x400u);
        if ( !vars0[i_0 - 45] )
        {
          perror("failed to allocate memory for chain_msg_ptr!\n");
          return 0;
        }
        memset((void *)vars0[i_0 - 45], 0, 0x400u);
      }
      ...
      chain_msg_len[0] = 0;
      vars0[freq_set_num - 29] = strlen(file_buf);
      for ( i_2 = 0; i_2 < freq_set_num; ++i_2 )
      {
        strncpy((char *)vars0[i_2 - 45], &file_buf[vars0[i_2 - 29]], vars0[i_2 - 28] - vars0[i_2 - 29]);
        if ( strstr((const char *)vars0[i_2 - 45], "\n\n") )
          strncpy((char *)vars0[i_2 - 45], (const char *)(vars0[i_2 - 45] + 2), vars0[i_2 - 28] - vars0[i_2 - 29]);
      }
      for ( i_3 = 0; i_3 <= 15; ++i_3 )
      {
        if ( chain_list[i_3] )
        {
          ret = conf_process(i_3, (uint16_t (*)[9])(*freq_data)[12 * i_3], (const char *)vars0[i_3 - 45]);
          if ( !ret )
            return 0;
...
// 23BC8: using guessed type _UNKNOWN *_frame_dummy_init_array_entry;
// 23BCC: using guessed type _UNKNOWN *_do_global_dtors_aux_fini_array_entry;
