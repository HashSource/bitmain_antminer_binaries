...

{
  ...
  printf("%s: Save MES system config information into %s\n","save_MES_system_submit_result",
         submit_result_file);
  snprintf(tmp1,0x800,"Save MES system config information into %s",submit_result_file);
  ...
  if (__s == (FILE *)0x0) {
    ...
    snprintf(tmp1,0x800,"Open %s fail",submit_result_file);
    log_to_file(tmp1,time_stamp);
    _Var2 = false;
  }
  else {
    ...
    snprintf(tmp1,0x800,"Open %s success",submit_result_file);
    log_to_file(tmp1,time_stamp);
    ...
    if (sVar1 == 1) {
      ...
      snprintf(tmp1,0x800,"Write %s success",submit_result_file);
      log_to_file(tmp1,time_stamp);
      fclose(__s);
      system("sync");
      _Var2 = true;
      ...
      snprintf(tmp1,0x800,"Write %s fail",submit_result_file);
      log_to_file(tmp1,time_stamp);
      fclose(__s);
      _Var2 = false;
    }
