...

{
  ...
  printf("%s: pre_voltage = %d, target_voltage = %d, step_num = %d\n","set_dac_voltage_step_by_step"
         ,pre_voltage,target_voltage,(uint)step_num);
  ...
  if (_Var1) {
    ...
    if (_Var1) {
      ...
      snprintf(tmp1,0x800,"pre_N = %d, target_N = %d, N_gap = %d, step_length = %d",pre_N,target_N,
      ...
      if (step_length == '\0') {
        ...
        snprintf(tmp1,0x800,"setting_N = %d",setting_N);
        log_to_file(tmp1,time_stamp);
        ...
        for (i = '\0'; i < step_num; i = i + '\x01') {
          ...
          snprintf(tmp1,0x800,"setting_N = %d, step=%d",setting_N,(uint)i);
          log_to_file(tmp1,time_stamp);
          _Var1 = set_voltage_by_N(board_name,which_chain,setting_N);
          if (!_Var1) {
            lcd_clear_result();
            ...
            snprintf(tmp1,0x800,"Set adc voltage by N = %d fail",setting_N);
            log_to_file(tmp1,time_stamp);
            return false;
          }
          usleep(300000);
