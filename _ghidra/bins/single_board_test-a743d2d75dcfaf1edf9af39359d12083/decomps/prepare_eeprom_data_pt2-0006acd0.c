    ...
    if ((Local_Config_Information->Hash_Board).Sensor_Info.Read_Temperature_From_Pic != false) {
      ...
      if (temp == false) {
        ...
        printf("%s: %d PIC sensors are not all ok. They are: \n","prepare_eeprom_data_pt2",
               gPic_sensor_num);
        snprintf(tmp1,0x800,"%d PIC sensors are not all ok. They are: ",gPic_sensor_num);
        log_to_file(tmp1,time_stamp);
        for (i = 0; i < gPic_sensor_num; i = i + 1) {
      ...
      water_temp[1] = gHistory_Result[test_loop_index].sensor_local_temperature_from_pic[1];
      water_temp[0] = gHistory_Result[test_loop_index].sensor_local_temperature_from_pic[0];
      p_Var5 = ctx->pcb_temp_in_set;
      iVar2 = get_min_value(water_temp,'\x02');
      (*p_Var5)(peVar1,iVar2);
      peVar1 = ctx;
      p_Var5 = ctx->pcb_temp_out_set;
      iVar2 = get_max_value(water_temp,'\x02');
      (*p_Var5)(peVar1,iVar2);
    }
    peVar1 = ctx;
