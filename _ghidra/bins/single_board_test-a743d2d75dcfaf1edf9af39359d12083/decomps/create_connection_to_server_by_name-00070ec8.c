...

{
  ...
  if (iVar2 == 0) {
    ...
    printf("%s ",time_stamp);
    printf("%s: %s: %s\n","create_connection_to_server_by_name",&DAT_001de05c,ip);
    snprintf(tmp1,0x800,"%s: %s",&DAT_001de05c,ip);
    ...
    print_time_stamp(time_stamp,0x30);
    printf("%s ",time_stamp);
    printf("%s: Can\'t get %s ip\n","create_connection_to_server_by_name",&DAT_001de05c);
    snprintf(tmp1,0x800,"Can\'t get %s ip",&DAT_001de05c);
    log_to_file(tmp1,time_stamp);
  }
  return -1;
}
