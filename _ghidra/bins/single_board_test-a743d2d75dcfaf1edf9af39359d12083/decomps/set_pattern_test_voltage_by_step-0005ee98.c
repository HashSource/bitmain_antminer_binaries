...

{
  ...
  snprintf(tmp1,0x800,"pre_voltage = %d, target_voltage = %d, step_num = %d",pre_voltage,
           target_voltage,(uint)step_num);
  log_to_file(tmp1,time_stamp);
  if ((((gHistory_Result[gPattern_test_counter].power_version == 0x62) ||
       (gHistory_Result[gPattern_test_counter].power_version == 100)) ||
      (gHistory_Result[gPattern_test_counter].power_version == 0x65)) ||
     (gHistory_Result[gPattern_test_counter].power_version == 0x66)) {
    ...
    if (0 < iVar7) {
      ...
      for (i_1 = 1; i_1 < (int)(step_num_local + 1); i_1 = i_1 + 1) {
        ...
        print_time_stamp(time_stamp,0x30);
        printf("%s ",time_stamp);
        printf("%s: bitmain_set_voltage step %d to %f\n","set_pattern_test_voltage_by_step",i_1,&vol
               ,(double)vol);
        snprintf(tmp1,0x800,"bitmain_set_voltage step %d to %f",i_1,(double)vol);
        log_to_file(tmp1,time_stamp);
    ...
    snprintf(tmp1,0x800,"pre_N = %d, target_N = %d, N_gap = %d, step_length = %d",uVar4 & 0xff,
    ...
    if (bVar3 == 0) {
      ...
      snprintf(tmp1,0x800,"setting_N = %d",(uint)setting_N);
      log_to_file(tmp1,time_stamp);
      ...
      for (i = '\0'; i < step_num; i = i + '\x01') {
        ...
        if ((uVar4 != setting_N) &&
          ...
          printf("%s: Set APW voltage by N = %d fail\n","set_pattern_test_voltage_by_step",
                 (uint)setting_N);
          snprintf(tmp1,0x800,"Set APW voltage by N = %d fail",(uint)setting_N);
          log_to_file(tmp1,time_stamp);
          return false;
        }
      }
