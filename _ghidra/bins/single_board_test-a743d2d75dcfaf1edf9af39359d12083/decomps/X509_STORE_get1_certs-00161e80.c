...

{
  ...
  st_00 = sk_new_null();
  CRYPTO_lock(9,0xb,"x509_lu.c",0x1ed);
  iVar1 = x509_object_idx_cnt(st->ctx->objs,1,nm,&local_2c);
  if (iVar1 < 0) {
    CRYPTO_lock(10,0xb,"x509_lu.c",0x1f5);
    ...
    if (iVar1 != 0) {
      ...
      CRYPTO_lock(9,0xb,"x509_lu.c",0x1fb);
      iVar1 = x509_object_idx_cnt(st->ctx->objs,1,nm,&local_2c);
      if (-1 < iVar1) goto LAB_00161eb4;
      CRYPTO_lock(10,0xb,"x509_lu.c",0x1fe);
    }
    p_Var5 = (_STACK *)0x0;
    sk_free(st_00);
  }
...
LAB_00161eb4:
    if (0 < local_2c) {
      ...
      do {
        ...
        a = *(X509 **)((int)pvVar2 + 4);
        CRYPTO_add_lock(&a->references,1,3,"x509_lu.c",0x206);
        iVar3 = sk_push(st_00,a);
        if (iVar3 == 0) {
          CRYPTO_lock(10,0xb,"x509_lu.c",0x208);
          X509_free(a);
