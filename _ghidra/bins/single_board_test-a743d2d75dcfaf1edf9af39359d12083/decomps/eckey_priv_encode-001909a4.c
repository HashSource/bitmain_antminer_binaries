...

{
  ...
  if (iVar1 == 0) {
    ERR_put_error(0x10,0xd6,0x8e,"ec_ameth.c",0x137);
    iVar1 = 0;
  }
  else {
    ...
    if (iVar1 == 0) {
      ...
      ERR_put_error(0x10,0xd6,0x10,"ec_ameth.c",0x147);
      iVar1 = 0;
    }
    else {
      penc = (uchar *)CRYPTO_malloc(iVar1,"ec_ameth.c",0x14a);
      if (penc == (uchar *)0x0) {
        EC_KEY_set_enc_flags(key,flags);
        ERR_put_error(0x10,0xd6,0x41,"ec_ameth.c",0x14d);
        iVar1 = 0;
      }
      else {
        local_2c = penc;
        ...
        if (iVar2 == 0) {
          EC_KEY_set_enc_flags(key,flags);
          CRYPTO_free(penc);
          ERR_put_error(0x10,0xd6,0x10,"ec_ameth.c",0x154);
          iVar1 = 0;
        }
        else {
          EC_KEY_set_enc_flags(key,flags);
