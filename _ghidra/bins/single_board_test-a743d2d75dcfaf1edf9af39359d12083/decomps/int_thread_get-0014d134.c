...

{
  ...
  if (int_thread_hash == (_LHASH *)0x0) {
    if (param_1 == 0) goto LAB_0014d152;
    CRYPTO_push_info_("int_thread_get (err.c)","err.c",0x1c6);
    int_thread_hash = lh_new((LHASH_HASH_FN_TYPE)0x14d205,(LHASH_COMP_FN_TYPE)0x14d201);
    CRYPTO_pop_info();
    if (int_thread_hash == (_LHASH *)0x0) goto LAB_0014d152;
  }
