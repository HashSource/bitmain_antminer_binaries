  ...
  if (reg_value_buf->reg_value_num < 0x1ff) {
    if ((undefined1  [16])((undefined1  [16])*reg_buf & (undefined1  [16])0x60000000) ==
      ...
      if ((*(byte *)reg_buf & 0xf) == gChain) {
        ...
        reg_value_buf->reg_buffer[reg_value_buf->p_wr].chain_number = *(byte *)reg_buf & 0xf;
        ...
        snprintf(tmp1,0x800,"receive register value from chain: %d, but not gChain: %d",
      ...
      snprintf(tmp1,0x800,"gHashboard_received_crc_error_work = %d",
               gHashboard_received_crc_error_work);
      log_to_file(tmp1,time_stamp);
    }
  }
    ...
    snprintf(tmp1,0x800,"%s: reg_value_buf buffer is full!","BTC_check_register");
    log_to_file(tmp1,time_stamp);
    ...
    for (i = 0; i < 10; i = i + 1) {
      print_time_stamp(time_stamp,0x30);
      printf("%s ",time_stamp);
      uVar4 = reg_value_buf->reg_buffer[i + 500].reg_value;
      uVar1 = __aeabi_uidiv(reg_value_buf->reg_buffer[i + 500].chip_address,gAddress_interval);
      printf("%s: reg_value = 0x%08x, which_asic = %d, reg_address = 0x%08x\n","BTC_check_register",
             uVar4,uVar1,reg_value_buf->reg_buffer[i + 500].reg_address);
      uVar4 = reg_value_buf->reg_buffer[i + 500].reg_value;
      uVar1 = __aeabi_uidiv(reg_value_buf->reg_buffer[i + 500].chip_address,gAddress_interval);
      snprintf(tmp1,0x800,"reg_value = 0x%08x, which_asic = %d, reg_address = 0x%08x",uVar4,uVar1,
               reg_value_buf->reg_buffer[i + 500].reg_address);
      log_to_file(tmp1,time_stamp);
      uVar3 = (uint)gPattern_test_counter;
      iVar2 = __aeabi_uidiv(reg_value_buf->reg_buffer[i + 500].chip_address,gAddress_interval);
