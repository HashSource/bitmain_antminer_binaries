...

{
  ...
  snprintf(tmp1,0x800,"Start %s","read_temperature_from_PIC_N_times");
  ...
  while( true ) {
    ...
    for (i = '\0'; i < gPic_sensor_num; i = i + '\x01') {
      memset(data,0,2);
      iVar4 = pic_read_iic((uint8_t)gChain,gPic_sensor_i2c_addr + gPic_sensor_low_3_bits_addr[i],
