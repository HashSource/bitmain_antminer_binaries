...

{
  ...
  snprintf(tmp1,0x800,"Start %s","read_temperature_from_PIC");
  log_to_file(tmp1,time_stamp);
  while (gStart_read_temp_from_pic != false) {
    for (i = '\0'; i < gPic_sensor_num; i = i + '\x01') {
      memset(data,0,2);
      iVar2 = pic_read_iic((uint8_t)gChain,gPic_sensor_i2c_addr + gPic_sensor_low_3_bits_addr[i],
      ...
      if (iVar2 == 2) {
        ...
        snprintf(tmp1,0x800,"Read PIC sensor %d fail",(uint)i);
        log_to_file(tmp1,time_stamp);
      }
    }
    for (i = '\0'; i < gPic_sensor_num; i = i + '\x01') {
      ...
      printf("%s: Read from Pic ::    Sensor %d:      local: %d\n","read_temperature_from_PIC",
             (uint)i,(int)gHistory_Result[gPattern_test_counter].sensor_local_temperature_from_pic
                          [i]);
      snprintf(tmp1,0x800,"Read from Pic ::    Sensor %d:      local: %d",(uint)i,
               (int)gHistory_Result[gPattern_test_counter].sensor_local_temperature_from_pic[i]);
      log_to_file(tmp1,time_stamp);
    }
    _Var1 = check_temperature();
    if (!_Var1) {
      gHistory_Result[gPattern_test_counter].temp_abnormal_count =
           gHistory_Result[gPattern_test_counter].temp_abnormal_count + '\x01';
    }
    sleep_counter = '\0';
    while ((gStart_read_temp_from_pic != false &&
           (uVar3 = (uint)sleep_counter, sleep_counter = sleep_counter + '\x01',
           uVar3 < gRead_Pic_Temperature_time_interval))) {
      sleep(1);
    }
  }
  print_time_stamp(time_stamp,0x30);
  printf("%s ",time_stamp);
  printf("%s: %s stop\n","read_temperature_from_PIC","read_temperature_from_PIC");
  snprintf(tmp1,0x800,"%s stop","read_temperature_from_PIC");
  log_to_file(tmp1,time_stamp);
  return (void *)0x0;
}
