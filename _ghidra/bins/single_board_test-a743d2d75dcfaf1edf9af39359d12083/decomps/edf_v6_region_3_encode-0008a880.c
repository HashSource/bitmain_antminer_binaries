...
int edf_v6_region_3_encode(edf_ctx_p ctx,char *dst,size_t *len,size_t *offset)

{
  ...
  if (*len < 0x88) {
    ...
    printf("%s ",time_stamp);
    printf("%s: INPUT too short %u<%u\n","edf_v6_region_3_encode",*len,0x88);
    ...
    *(uint8_t *)((int)pvVar3 + 0xf9) = uVar1;
    ...
    if (_Var2) {
      ...
      *offset = 0x72;
      ...
      print_time_stamp(time_stamp,0x30);
      printf("%s ",time_stamp);
      printf("%s: %s: data_enc error\n","edf_v6_region_3_encode","edf_v6_region_3_encode");
      snprintf(tmp1,0x800,"%s: data_enc error","edf_v6_region_3_encode");
      log_to_file(tmp1,time_stamp);
      iVar4 = -1;
    }
  }
