...

{
  ...
  printf("%s ",time_stamp);
  dVar5 = (double)freq_step;
  printf("%s: fixed step freq_start = %d, freq_end = %d, freq_step = %.2f\n\n",
         "inc_freq_with_adjust_voltage",freq_start,freq_end,dVar5);
  snprintf(tmp1,0x800,"fixed step freq_start = %d, freq_end = %d, freq_step = %.2f\n",freq_start,
           freq_end,(int)((ulonglong)dVar5 >> 0x20),(double)freq_step);
  ...
  for (step_id = 1; step_id <= step_count; step_id = step_id + 1) {
    ...
    if ((freq_start < freq_end) && (300.0 <= curr_freq)) {
      ...
      snprintf(tmp1,0x800,"inc freq get min board temp: %d",(int)sVar1);
      log_to_file(tmp1,time_stamp);
      if ((0x23 < sVar1) && (step_id < step_count)) {
        if (avg == 0) {
        ...
        iVar4 = gHistory_Result[gPattern_test_counter].pre_open_core_voltage - idx * avg;
        idx = idx + 1;
        print_time_stamp(time_stamp,0x30);
        printf("%s ",time_stamp);
        printf("%s: relax voltage %d\n","inc_freq_with_adjust_voltage",iVar4);
        snprintf(tmp1,0x800,"relax voltage %d",iVar4);
        log_to_file(tmp1,time_stamp);
        iVar3 = bitmain_set_voltage((double)(longlong)iVar4 / 100.0);
        if (iVar3 != 0) {
          print_time_stamp(time_stamp,0x30);
          printf("%s ",time_stamp);
          printf("%s: bitmain_set_voltage %d ret=%d\n","inc_freq_with_adjust_voltage",iVar4,iVar3);
          snprintf(tmp1,0x800,"bitmain_set_voltage %d ret=%d",iVar4,iVar3);
          log_to_file(tmp1,time_stamp);
        }
      }
    }
