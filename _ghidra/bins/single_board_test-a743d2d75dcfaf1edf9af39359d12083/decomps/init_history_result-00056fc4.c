...

{
  ...
  if (test_loop_counter == 0) {
    memset(gHistory_Result,0,0xcbc410);
  }
  ...
  gHistory_Result[test_loop_counter].asic_sensor_all_OK = false;
  gHistory_Result[test_loop_counter].apw_power_ok = true;
  gHistory_Result[test_loop_counter].pic_ok = true;
  gHistory_Result[test_loop_counter].switch_OK = true;
  gHistory_Result[test_loop_counter].sensor_calibration_OK = true;
  ...
  memset(gHistory_Result[test_loop_counter].asic_sensor_OK,1,4);
  memset(gHistory_Result[test_loop_counter].sensor_local_temperature_from_asic,-0x7f,4);
  memset(gHistory_Result[test_loop_counter].sensor_remote_temperature_from_asic,-0x7f,4);
  ...
  gHistory_Result[test_loop_counter].temp_abnormal_count = '\0';
  *(undefined4 *)&gHistory_Result[test_loop_counter].nonce_rate = 0;
  *(undefined4 *)((int)&gHistory_Result[test_loop_counter].nonce_rate + 4) = 0;
  gHistory_Result[test_loop_counter].valid_nonce_num = 0;
  gHistory_Result[test_loop_counter].repeat_nonce_number = 0;
