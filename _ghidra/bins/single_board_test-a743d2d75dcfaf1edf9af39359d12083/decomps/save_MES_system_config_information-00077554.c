...

{
  ...
  if (which_information == 0) {
    ...
    if (which_information != 1) {
      ...
      snprintf(tmp1,0x800,"Don\'t support save this type information. type = %d",which_information);
  ...
  printf("%s: Save MES system config information into %s\n","save_MES_system_config_information",
         factory_config_file);
  snprintf(tmp1,0x800,"Save MES system config information into %s",factory_config_file);
  log_to_file(tmp1,time_stamp);
  ...
  if (__s == (FILE *)0x0) {
    ...
    snprintf(tmp1,0x800,"Open %s fail",factory_config_file);
    log_to_file(tmp1,time_stamp);
    _Var2 = false;
  }
  else {
    ...
    snprintf(tmp1,0x800,"Open %s success",factory_config_file);
    log_to_file(tmp1,time_stamp);
    ...
    if (sVar1 == 1) {
      ...
      snprintf(tmp1,0x800,"Write %s success",factory_config_file);
      log_to_file(tmp1,time_stamp);
      fclose(__s);
      _Var2 = true;
    }
      ...
      snprintf(tmp1,0x800,"Write %s fail",factory_config_file);
      log_to_file(tmp1,time_stamp);
      fclose(__s);
      _Var2 = false;
    }
