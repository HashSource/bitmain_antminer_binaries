...

{
  ...
  if (uVar2 == 0x10000000) {
    Curl_failf(iVar4,"operation aborted by callback");
    uVar3 = 0x2a;
    *param_3 = 0;
  }
  else if (uVar2 == 0x10000001) {
    if (*(int *)(param_1[0x86] + 0x40) << 0x1b < 0) {
      Curl_failf(iVar4,"Read callback asked for PAUSE when not supported!");
      uVar3 = 0x1a;
    }
    else {
    ...
    *param_3 = 0;
    Curl_failf(iVar4,"read function returned funny value");
    uVar3 = 0x1a;
  }
  else {
    if ((*(char *)(iVar4 + 0x14b) == '\0') && (*(char *)(iVar4 + 0x149) != '\0')) {
      ...
      __n = curl_msnprintf(auStack_2c,0xb,&DAT_0013787c,uVar2,__s);
      sVar5 = uVar2 + __n;
