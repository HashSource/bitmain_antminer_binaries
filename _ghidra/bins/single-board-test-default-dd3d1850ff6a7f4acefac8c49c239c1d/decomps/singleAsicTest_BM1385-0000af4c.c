...

{
  ...
  if (gChain == 0xff) {
    ...
    reset_hash_board();
    start_receive = 1;
    pthread_create((pthread_t *)(cgpu + 0x10004),(pthread_attr_t *)0x0,(__start_routine *)0xac59,
                   chain_info + (uint)gChain * 2);
    puts("\n--- check asic number");
    ...
    if ((uint)(byte)cgpu[gChain + 0x20338] == Conf._108_4_) {
      ...
      putchar(10);
      gStartTest = 1;
      pthread_create((pthread_t *)(cgpu + 0x10000),(pthread_attr_t *)0x0,(__start_routine *)0xa82d,
                     chain_info + (uint)gChain * 2);
