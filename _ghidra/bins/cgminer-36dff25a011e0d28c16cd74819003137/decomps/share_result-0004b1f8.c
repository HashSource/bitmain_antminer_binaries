...

{
  ...
  if (((res == (json_t *)0x0) || (res->type != JSON_TRUE)) &&
    ...
    uVar5 = (uint)pool_00->rejected;
    iVar6 = *(int *)((int)&pool_00->rejected + 4);
    *(uint *)&pool_00->rejected = uVar5 + 1;
    *(uint *)((int)&pool_00->rejected + 4) = iVar6 + (uint)(0xfffffffe < uVar5);
    ...
    if ((opt_quiet != true) && (opt_realquiet != true)) {
      ...
      if (res_local == (json_t *)0x0) {
        if ((work->stratum != false) && (err != (json_t *)0x0)) {
          if ((err == (json_t *)0x0) || (err->type != JSON_ARRAY)) {
            if ((err != (json_t *)0x0) && (err->type == JSON_STRING)) {
              pcVar3 = json_string_value(err);
              snprintf(reason,0x1f," (%s)",pcVar3);
            }
          }
          else {
        ...
        pcVar3 = json_string_value(res_local);
        reasonLen = strlen(pcVar3);
        if (0x1c < reasonLen) {
          reasonLen = 0x1c;
        }
        reason[0] = ' ';
        reason[1] = '(';
        _cg_memcpy(reason + 2,pcVar3,reasonLen,"cgminer.c","share_result",0xe1a);
        reason[reasonLen + 2] = ')';
        reason[reasonLen + 3] = '\0';
        _cg_memcpy(disposition + 7,pcVar3,reasonLen,"cgminer.c","share_result",0xe1f);
        disposition[6] = ':';
      ...
      if (((use_syslog != false) || (opt_log_output != false)) || (4 < opt_log_level)) {
        ...
        snprintf(tmp42,0x800,"Rejected %s %s %d %s%s %s%s version 0x%x",hashshow,pcVar1->drv->name,
                 pcVar1->device_id,where,reason,uVar5 | 0xa0000,worktime,work->version);
    ...
    uVar5 = (uint)pool_00->accepted;
    iVar6 = *(int *)((int)&pool_00->accepted + 4);
    *(uint *)&pool_00->accepted = uVar5 + 1;
    *(uint *)((int)&pool_00->accepted + 4) = iVar6 + (uint)(0xfffffffe < uVar5);
    ...
    if ((opt_quiet != true) && (opt_realquiet != true)) {
      if (total_pools < 2) {
        ...
        snprintf(tmp42,0x800,"Accepted %s %s %d pool %d %s%s",hashshow,pcVar1->drv->name,
                 pcVar1->device_id,work->pool->pool_no,uVar5 | 0xa0000,worktime);
        _applog(5,tmp42,false);
      }
    }
