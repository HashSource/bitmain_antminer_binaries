...

{
  ...
  for (i = 0; i < 0x10; i = i + 1) {
    ...
    for (j = 0; j < dev->chain_asic_temp_num[i]; j = j + 1) {
      iVar3 = sprintf(temp_pcb_value + len,"%d",(int)dev->chain_asic_temp[i][j][0]);
      len = len + iVar3;
      if (j + 1 < (int)dev->chain_asic_temp_num[i]) {
        pcVar6 = temp_pcb_value + len;
        pcVar6[0] = '-';
        pcVar6[1] = '\0';
        len = len + 1;
      }
  ...
  for (i = 0; i < 0x10; i = i + 1) {
    ...
    for (j_1 = 0; j_1 < dev->chain_asic_temp_num[i]; j_1 = j_1 + 1) {
      iVar3 = sprintf(temp_pcb_value + len_1,"%d",(int)dev->chain_asic_temp[i][j_1][1]);
      len_1 = len_1 + iVar3;
      if (j_1 + 1 < (int)dev->chain_asic_temp_num[i]) {
        pcVar6 = temp_pcb_value + len_1;
        pcVar6[0] = '-';
        pcVar6[1] = '\0';
        len_1 = len_1 + 1;
      }
    ...
    if (dev->chain_exist[i] == 1) {
      if (last_freq[i][1] == '}') {
        for (j_2 = 0; j_2 < (int)(uint)dev->chain_asic_num[i]; j_2 = j_2 + 1) {
          iVar3 = freq_pll_1391[0xc].freq;
          if (chain_badcore_num[i][j_2] < 0xf) {
            iVar3 = freq_pll_1391[*(byte *)(i * 0x100 + j_2 * 2 + 0x3aeccf)].freq;
          }
    ...
    if ((dev->chain_exist[i] == 1) && (last_freq[i][1] == '}')) {
      for (j_3 = 0; j_3 < (int)(uint)dev->chain_asic_num[i]; j_3 = j_3 + 1) {
        ...
        if (chain_badcore_num[i][j_3] < 0xf) {
          iVar3 = freq_pll_1391[*(byte *)(i * 0x100 + j_3 * 2 + 0x3aeccf)].freq;
        }
  ...
  total_diff1 = __fixdfdi(SUB84(dVar8,0),(int)((ulonglong)dVar8 >> 0x20));
  if (total_diff1 + hw_errors == 0) {
    dev_hwp = 0.0;
  }
  else {
    lVar1 = (longlong)hw_errors;
    dVar8 = (double)__aeabi_l2d((int)(total_diff1 + hw_errors),
  ...
  for (i = 0; i < 0x10; i = i + 1) {
    if (dev->chain_exist[i] == 1) {
      ...
      (temp_pcb_value + sVar4)[0] = '}';
      (temp_pcb_value + sVar4)[1] = '\0';
