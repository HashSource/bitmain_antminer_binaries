
void hash_driver_work(undefined4 *param_1)

{
  undefined *puVar1;
  char cVar2;
  longlong lVar3;
  int iVar4;
  int *piVar5;
  undefined4 uVar6;
  uint uVar7;
  undefined *puVar8;
  char *__format;
  undefined *puVar9;
  int iVar10;
  uint uVar11;
  int iVar12;
  bool bVar13;
  longlong lVar14;
  int local_838;
  int local_834;
  int iStack_830;
  int iStack_82c;
  char acStack_828 [2052];
  
  iVar10 = param_1[9];
  uVar6 = *param_1;
  lVar3 = 0;
  iVar12 = *(int *)(iVar10 + 4);
  local_838 = 0;
  local_834 = 0;
LAB_00025790:
  cVar2 = *(char *)(iVar10 + 0x16c);
  do {
    if (cVar2 != '\0') {
      *(undefined4 *)(iVar10 + 0x20) = 1;
      return;
    }
    *(undefined1 *)((int)param_1 + 0x3f) = 0;
    lVar14 = (**(code **)(iVar12 + 0x44))(param_1);
    *(undefined1 *)((int)param_1 + 0x3e) = 0;
    if (lVar14 == -1) {
      if (((use_syslog != '\0') || (opt_log_output != '\0')) || (2 < opt_log_level)) {
        snprintf(acStack_828,0x800,"%s %d failure, disabling!",*(undefined4 *)(iVar12 + 8),
                 *(undefined4 *)(iVar10 + 8));
        _applog(3,acStack_828,0);
      }
      *(undefined4 *)(iVar10 + 0x20) = 1;
      dev_error(iVar10);
      *(undefined4 *)(iVar10 + 0x20) = 1;
      return;
    }
    lVar3 = lVar14 + lVar3;
    uVar11 = (uint)((ulonglong)lVar3 >> 0x20);
    cgtime(&iStack_830);
    puVar9 = (undefined *)(iStack_82c - local_834);
    puVar8 = (undefined *)(iStack_830 - local_838);
    if ((int)puVar9 < 0) {
      puVar8 = puVar8 + -1;
      puVar9 = &DAT_000f4240 + (int)puVar9;
    }
    uVar7 = (uint)lVar3 | uVar11;
    if (uVar7 == 0) {
LAB_0002584a:
      if (opt_log_interval <= (int)puVar8) goto LAB_000257e4;
      lVar14 = lVar3;
      if (*(char *)(param_1 + 0xf) == '\0') goto LAB_00025800;
LAB_0002585a:
      mt_disable_isra_16(param_1,uVar6,iVar12 + 0x5c);
    }
    else {
      bVar13 = puVar8 == (undefined *)0x0;
      puVar1 = puVar8;
      if ((int)puVar8 < 1) {
        puVar1 = puVar9 + -200000;
        bVar13 = puVar9 == (undefined *)0x30d40;
      }
      if (bVar13 || (int)puVar1 < 0 != ((int)puVar8 < 1 && SBORROW4((int)puVar9,200000)))
      goto LAB_0002584a;
LAB_000257e4:
      hashmeter(uVar6,uVar7,(uint)lVar3,uVar11);
      copy_time(&local_838,&iStack_830);
      lVar3 = 0;
      lVar14 = 0;
      if (*(char *)(param_1 + 0xf) != '\0') goto LAB_0002585a;
LAB_00025800:
      lVar3 = lVar14;
      if (*(int *)(iVar10 + 0x20) != 0) goto LAB_0002585a;
    }
    if (*(char *)((int)param_1 + 0x3f) == '\0') goto LAB_00025790;
    iVar4 = pthread_mutex_lock((pthread_mutex_t *)update_job_lock);
    if (iVar4 != 0) {
      piVar5 = __errno_location();
      iVar10 = *piVar5;
      __format = "WTF MUTEX ERROR ON LOCK! errno=%d in %s %s():%d";
      uVar6 = 0x25bc;
LAB_000258ee:
      snprintf(acStack_828,0x800,__format,iVar10,"cgminer.c","hash_driver_work",uVar6);
      _applog(3,acStack_828,1);
                    /* WARNING: Subroutine does not return */
      __quit(1);
    }
    (**(code **)(iVar12 + 0x50))(iVar10);
    iVar4 = pthread_mutex_unlock((pthread_mutex_t *)update_job_lock);
    if (iVar4 != 0) {
      piVar5 = __errno_location();
      iVar10 = *piVar5;
      __format = "WTF MUTEX ERROR ON UNLOCK! errno=%d in %s %s():%d";
      uVar6 = 0x25be;
      goto LAB_000258ee;
    }
    (*selective_yield)();
    cVar2 = *(char *)(iVar10 + 0x16c);
  } while( true );
}

