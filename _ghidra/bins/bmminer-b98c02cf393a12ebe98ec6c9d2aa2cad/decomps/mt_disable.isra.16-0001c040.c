
void mt_disable_isra_16(int param_1,undefined4 param_2,undefined4 *param_3)

{
  char cVar1;
  int iVar2;
  char local_820 [28];
  undefined2 uStack_804;
  undefined1 local_802;
  
  if (((use_syslog != '\0') || (opt_log_output != '\0')) || (3 < opt_log_level)) {
    snprintf(local_820,0x800,"Thread %d being disabled",param_2);
    _applog(4,local_820,0);
  }
  cVar1 = opt_debug;
  iVar2 = *(int *)(param_1 + 0x24);
  *(undefined4 *)(iVar2 + 0x30) = 0;
  *(undefined4 *)(iVar2 + 0x34) = 0;
  if ((cVar1 != '\0') && (((use_syslog != '\0' || (opt_log_output != '\0')) || (6 < opt_log_level)))
     ) {
    builtin_strncpy(local_820,"Waiting on sem in miner thre",0x1c);
    uStack_804 = 0x6461;
    local_802 = 0;
    _applog(7,local_820,0);
  }
  _cgsem_wait(param_1 + 0x10,"cgminer.c","mt_disable",0x2359);
  if (((use_syslog != '\0') || (opt_log_output != '\0')) || (3 < opt_log_level)) {
    snprintf(local_820,0x800,"Thread %d being re-enabled",param_2);
    _applog(4,local_820,0);
  }
  (*(code *)*param_3)(param_1);
  return;
}

