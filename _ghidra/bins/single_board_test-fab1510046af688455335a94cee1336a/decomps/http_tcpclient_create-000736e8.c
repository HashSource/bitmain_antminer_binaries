...

{
  ...
  while ((i < 5 && (iVar3 = getaddrinfo(host,sockaddr_port,(addrinfo *)&hints,(addrinfo **)&servinfo
    ...
    printf("%s ",time_stamp);
    printf("%s: Failed to getaddrinfo for %s:%s, sleep\n","http_tcpclient_create",host,sockaddr_port
          );
    snprintf(tmp1,0x800,"Failed to getaddrinfo for %s:%s, sleep",host,sockaddr_port);
...
LAB_00073c3a:
      if (p == (addrinfo *)0x0) {
        ...
        printf("%s: Failed to connect to server on %s:%s\n\n","http_tcpclient_create",host,
               sockaddr_port);
        snprintf(tmp1,0x800,"Failed to connect to server on %s:%s\n",host,sockaddr_port);
        log_to_file(tmp1,time_stamp);
          ...
          if (0 < iVar3) {
            ...
            if ((1 << (uVar6 & 0xff) & *(uint *)(time_stamp + ((int)uVar5 >> 5) * 4)) != 0) {
              ...
              if ((iVar4 == 0) && (err == 0)) {
                ...
                snprintf(tmp1,0x800,"Succeeded delayed connect, count:%d\n",count);
