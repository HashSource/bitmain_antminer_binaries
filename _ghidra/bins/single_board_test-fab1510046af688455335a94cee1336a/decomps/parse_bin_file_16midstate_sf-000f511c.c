...

{
  ...
  while( true ) {
    ...
    if (sVar1 != 0xc) break;
    pwVar3 = pattern_info_new.works[asic] + subid + count * core;
    pwVar3->id = (uint16_t)subid;
    uVar2 = ntohl(work_in_file.nonce);
    (pwVar3->pattern).nonce = uVar2;
    *(undefined4 *)(pwVar3->pattern).midstate = work_in_file.midstate._0_4_;
    *(undefined4 *)((pwVar3->pattern).midstate + 4) = work_in_file.midstate._4_4_;
    subid = subid + 1;
  }
