...
int edf_v6_load(edf_ctx_p ctx,uint8_t *src,size_t len,int flag)

{
  ...
  pvVar7 = ctx->priv;
  bVar1 = *src;
  uVar4 = edf_v6_version_get(ctx);
  if (bVar1 != uVar4) {
    print_time_stamp(time_stamp,0x30);
    printf("%s ",time_stamp);
    bVar1 = *src;
    iVar5 = edf_v6_version_get(ctx);
    printf("%s: version invalid %d!=%d\n","edf_v6_load",(uint)bVar1,iVar5);
    bVar1 = *src;
    iVar5 = edf_v6_version_get(ctx);
    snprintf(tmp1,0x800,"version invalid %d!=%d",(uint)bVar1,iVar5);
    log_to_file(tmp1,time_stamp);
    return -1;
  }
  uVar6 = edf_v6_capability(ctx);
  if ((flag & uVar6) == 0) {
    print_time_stamp(time_stamp,0x30);
    printf("%s ",time_stamp);
