...

{
  ...
  if (iVar2 == 7) {
    ...
    if ((read_back_data[0] == 0x15) && (read_back_data[1] == 1)) goto LAB_001cbbee;
    snprintf(tmp42,0x800,"%s failed! read_back_data[0] = 0x%02x, read_back_data[1] = 0x%02x\n\n",
             "_bitmain_pic_disable_dc_dc_common",(uint)read_back_data[0],(uint)read_back_data[1]);
    _applog(0,tmp42,false);
  }
  else {
    __format = "%s write iic err\n";
LAB_001cbc38:
    snprintf(tmp42,0x800,__format,"_bitmain_pic_disable_dc_dc_common");
    _applog(0,tmp42,false);
  }
  iVar3 = -0x7ffffe00;
LAB_001cbbee:
