...

{
  ...
  if (iVar1 != 0) {
    ...
    if (local_1c == 0x10) {
      ...
      if (dh == (DH *)0x0) {
        ERR_put_error(5,0x6c,0x68,"dh_ameth.c",0x7b);
      }
      else {
        ai = d2i_ASN1_INTEGER((ASN1_INTEGER **)0x0,&puStack_28,local_20);
        if (ai == (ASN1_INTEGER *)0x0) {
          ERR_put_error(5,0x6c,0x68,"dh_ameth.c",0x80);
        }
        else {
          ...
          ERR_put_error(5,0x6c,0x6d,"dh_ameth.c",0x86);
          ASN1_INTEGER_free(ai);
        }
        DH_free(dh);
      }
      ...
      ERR_put_error(5,0x6c,0x69,"dh_ameth.c",0x72);
    }
  }
  return 0;
}
