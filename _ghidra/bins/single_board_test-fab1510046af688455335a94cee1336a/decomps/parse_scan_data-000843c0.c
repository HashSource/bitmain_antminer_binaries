...

{
  ...
  if ((((scan_data == (char *)0x0) || (sn == (hashboard_sn *)0x0)) || (sn_ok == (_Bool *)0x0)) ||
    ...
    snprintf(tmp1,0x800,"scan_code_data::%s",scan_data);
    log_to_file(tmp1,time_stamp);
    memset(data,0,0x100);
    for (i = 0; sVar3 = strlen(scan_data), i < sVar3; i = i + 1) {
    ...
    snprintf(tmp1,0x800,"after check, scan_code_data::%s",data);
    log_to_file(tmp1,time_stamp);
    pcVar4 = strchr(data,0x20);
    if (pcVar4 == (char *)0x0) {
      sVar3 = strlen(data);
      if (sVar3 != 0x11) {
        print_time_stamp(time_stamp,0x30);
      ...
      *sn_ok = true;
      ...
      snprintf(tmp1,0x800,"recv sn data::[%s]",sn->board_sn);
      log_to_file(tmp1,time_stamp);
    }
    else {
      *qrcode_ok = true;
      _Var1 = scan_multiple_ft_bin_info(data,qrcode);
      if (_Var1) {
        print_time_stamp(time_stamp,0x30);
        printf("%s ",time_stamp);
        printf("%s: is multiple ft_bin ver!\n","parse_scan_data");
        builtin_strncpy(tmp1,"is multiple ft_bin ver!",0x18);
        log_to_file(tmp1,time_stamp);
      }
      else {
      ...
      snprintf(tmp1,0x800,"factory_job::[%s].",qrcode);
      log_to_file(tmp1,time_stamp);
      print_time_stamp(time_stamp,0x30);
      printf("%s ",time_stamp);
      printf("%s: chip_die::[%s].\n","parse_scan_data",qrcode->chip_die);
      snprintf(tmp1,0x800,"chip_die::[%s].",qrcode->chip_die);
      log_to_file(tmp1,time_stamp);
      print_time_stamp(time_stamp,0x30);
      printf("%s ",time_stamp);
      printf("%s: chip_marking::[%s].\n","parse_scan_data",qrcode->chip_marking);
      snprintf(tmp1,0x800,"chip_marking::[%s].",qrcode->chip_marking);
      log_to_file(tmp1,time_stamp);
      if ((qrcode->ft_bin_list).ft_bin_cnt == 0) {
        ...
        log_out_multiple_ft_bin_info(&qrcode->ft_bin_list);
      }
      print_time_stamp(time_stamp,0x30);
      printf("%s ",time_stamp);
      printf("%s: temp_sensor_type::[%s].\n","parse_scan_data",qrcode->temp_sensor_type);
      snprintf(tmp1,0x800,"temp_sensor_type::[%s].",qrcode->temp_sensor_type);
      log_to_file(tmp1,time_stamp);
      print_time_stamp(time_stamp,0x30);
      printf("%s ",time_stamp);
      printf("%s: hashboard_ctrl_code::[%s].\n","parse_scan_data",qrcode->hashboard_ctrl_code);
      snprintf(tmp1,0x800,"hashboard_ctrl_code::[%s].",qrcode->hashboard_ctrl_code);
      log_to_file(tmp1,time_stamp);
      print_time_stamp(time_stamp,0x30);
      printf("%s ",time_stamp);
      printf("%s: hashboard_index::[%s].\n","parse_scan_data",qrcode->hashboard_index);
      snprintf(tmp1,0x800,"hashboard_index::[%s].",qrcode->hashboard_index);
      log_to_file(tmp1,time_stamp);
    }
    iVar5 = 0;
  }
