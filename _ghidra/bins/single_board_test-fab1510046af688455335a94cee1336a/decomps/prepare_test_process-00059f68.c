...

{
  ...
  if (iVar2 == 0) {
    if ((Local_Config_Information->Hash_Board).Asic_Num == 1) {
      ...
      builtin_strncpy(tmp1,"Single ASIC don\'t support PT1 te",0x20);
      tmp1[0x20] = 's';
      tmp1[0x21] = 't';
      tmp1[0x22] = '\0';
      ...
      tmp1._32_2_ = (ushort)(byte)tmp1[0x21] << 8;
      log_to_file(tmp1,time_stamp);
      _Var1 = true;
    }
  }
      ...
      if (iVar2 == 0) {
        ...
        if ((((iVar2 == 0) ||
           ...
           ((iVar2 = strcmp(Local_Config_Information->Test_Process,"PT3"), iVar2 == 0 ||
            (iVar2 = strcmp(Local_Config_Information->Test_Process,"PT1&PT3"), iVar2 == 0)))) {
          (Local_Config_Information->Repair_Mode).Enable_Repair = false;
          if ((Local_Config_Information->Hash_Board).Asic_Num == 1) {
            display_main_page();
            print_time_stamp(time_stamp,0x30);
            printf("%s ",time_stamp);
            ...
            if ((Local_Config_Information->Test_Info).Factory_Mode == false) {
              ...
              if (!_Var1) {
                ...
                builtin_strncpy(tmp1,"Prepare all config handshake fai",0x20);
                tmp1[0x20] = 'l';
                tmp1[0x21] = '\0';
                log_to_file(tmp1,time_stamp);
                return false;
