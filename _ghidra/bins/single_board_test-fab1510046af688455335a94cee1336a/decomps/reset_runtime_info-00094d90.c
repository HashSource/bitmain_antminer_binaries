...

{
  ...
  char tmp42 [256];
  uint16_t every_asic_reg_last_nonce_num [256];
  pattern_work *works;
  pattern_work *work;
  ...
  memcpy(every_asic_reg_last_nonce_num,g_rt.asic_stats.every_asic_reg_last_nonce_num,0x200);
  memset(&g_rt.asic_stats,0,0x52284);
  memcpy(g_rt.asic_stats.every_asic_reg_last_nonce_num,every_asic_reg_last_nonce_num,0x200);
  g_rt.chain_status = 0;
