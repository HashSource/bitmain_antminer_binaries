...

{
  ...
  for (which_asic = 0; which_asic < uVar1; which_asic = which_asic + 1) {
    pwVar2 = pattern_info.works[which_asic];
    for (which_core = 0; which_core < (Local_Config_Information->Hash_Board).Small_Core_Num;
      ...
      for (which_pattern = 0;
        ...
        printf("work id : %d, is_nonce_return_back = %d",(uint)pwVar3->id,
               (uint)pwVar3->is_nonce_return_back);
        printf("\nmidstate=0x");
        for (i = 0; i < 0x20; i = i + 1) {
          printf("%02x",(uint)(pwVar3->pattern).midstate[i]);
        }
        printf("\ndata2=");
        for (i = 0; i < 0xc; i = i + 1) {
          printf("%02x",(uint)(pwVar3->pattern).data[i]);
        }
        printf("\nnonce=0x%08x",(pwVar3->pattern).nonce);
        putchar(10);
      }
    }
  }
