...

{
  ...
  if (((uint)param_1->srtp_profile & 3) != 1) {
    ERR_put_error(0x14,0x13b,0x16d,"t1_lib.c",0xf43);
    return -1;
  }
  if (param_1->tlsext_heartbeat != 0) {
    ERR_put_error(0x14,0x13b,0x16e,"t1_lib.c",0xf49);
    return -1;
  }
  uVar1 = SSL_state(param_1);
  if (((uVar1 & 0x3000) != 0) || (param_1->in_handshake != 0)) {
    ERR_put_error(0x14,0x13b,0xf4,"t1_lib.c",0xf4f);
    return -1;
  }
  buf = (undefined1 *)CRYPTO_malloc(0x25,"t1_lib.c",0xf63);
  *buf = 1;
  buf[1] = 0;
  buf[2] = 0x12;
