...

{
  ...
  register_value = 0;
  asic_name = 0;
  g_rt.pause_read_asic_temp = 1;
  while (g_rt.enable_get_register == 0) {
    usleep(10000);
  }
  clear_register_buf();
    ...
    for (i = 0; i < uVar3; i = i + 1) {
      ...
      case 0:
        if (register_value >> 0x10 == (uint)asic_name) {
          ...
          snprintf(tmp1,0x800,"chip address register is not 0x%04x, but is 0x%04x",(uint)asic_name,
                   register_value >> 0x10);
          log_to_file(tmp1,time_stamp);
      ...
      default:
        ...
        snprintf(tmp1,0x800,"Don\'t support this register in check: 0x%02x",which_function_local);
        log_to_file(tmp1,time_stamp);
        return 0xffffffff;
      case 3:
        print_time_stamp(time_stamp,0x30);
      ...
      case 0xf:
        iVar2 = __aeabi_uidiv(reg_value_buf->reg_buffer[reg_value_buf->p_rd].chip_address,
                              (g_rt.config)->interval);
        g_rt.asic_stats.every_asic_reg_tick_nonce_num[iVar2] = uVar1;
      }
      pthread_mutex_lock((pthread_mutex_t *)&reg_mutex);
