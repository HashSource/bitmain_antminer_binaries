...
int check_hw_err(pattern_work *work,uint32_t nonce)

{
  int iVar1;
  uint32_t nonce_local;
  pattern_work *work_local;
  sha2_context ctx;
  uint32_t nonce_temp;
  uint8_t hash2 [32];
  uint8_t hash1 [32];
  ...
  if (work == (pattern_work *)0x0) {
    iVar1 = 0;
  }
  else {
    memcpy(ctx.state,(work->pattern).midstate,0x20);
    rev((uchar *)ctx.state,0x20);
    ctx.total[0] = 0x50;
    ctx.total[1] = 0;
    memcpy(hash1,(work->pattern).data,0xc);
    rev(hash1,0xc);
