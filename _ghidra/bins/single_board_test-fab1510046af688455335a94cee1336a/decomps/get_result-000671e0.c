...
/* WARNING: Removing unreachable block (ram,0x00067932) */
/* WARNING: Removing unreachable block (ram,0x00067936) */
...

{
  ...
  if ((((iVar1 != 0) &&
     ...
     ((iVar1 = strcmp((Local_Config_Information->Hash_Board).Asic_Type,"BM1370"), iVar1 != 0 &&
      (iVar1 = strcmp((Local_Config_Information->Hash_Board).Asic_Type,"BM1370"), iVar1 != 0)))) {
    ...
    if (iVar1 == 0) {
      ...
      if (iVar1 != 0) {
        ...
        snprintf(tmp1,0x800,"Don\'t support this type ASIC: %s\n",
                 (Local_Config_Information->Hash_Board).Asic_Type);
    ...
    for (which_voltage_domain = 0; which_voltage_domain < uVar5;
      ...
      for (which_asic = 0; which_asic < uVar6; which_asic = which_asic + 1) {
        voltage_domain_received_nonce_number =
             ...
             [uVar6 * which_voltage_domain + which_asic] + voltage_domain_received_nonce_number;
        if (gHistory_Result[gPattern_test_counter].every_asic_got_nonce_num
            [uVar6 * which_voltage_domain + which_asic] < uVar3 * uVar4) {
          for (which_core = 0; which_core < statistics_core_num; which_core = which_core + 1) {
            iVar1 = strcmp((Local_Config_Information->Hash_Board).Asic_Type,"BM1399");
            if (((iVar1 != 0) || (which_core != 0x23)) &&
               ((iVar1 = strcmp((Local_Config_Information->Hash_Board).Asic_Type,"BM1362"),
                iVar1 != 0 || ((which_core < 0x25 || (0x27 < which_core)))))) {
              strcmp((Local_Config_Information->Hash_Board).Asic_Type,"BM1366");
              strcmp((Local_Config_Information->Hash_Board).Asic_Type,"BM1370");
              iVar1 = strcmp((Local_Config_Information->Hash_Board).Asic_Type,"BM1368");
              if ((iVar1 != 0) || (which_core < 0x4f0)) {
                strcmp(Local_Config_Information->Test_Process,"PT1+");
              }
            }
          }
          if (gHistory_Result[gPattern_test_counter].every_asic_got_nonce_num
      ...
      if (uVar6 < 6) {
        ...
        printf("domain[%02d]: ",which_voltage_domain);
        for (which_asic = 0; which_asic < uVar6; which_asic = which_asic + 1) {
          printf(" %7d ",gHistory_Result[gPattern_test_counter].every_asic_got_nonce_num
                         [uVar6 * which_voltage_domain + which_asic]);
          if ((which_asic != 0) && (__aeabi_uidivmod(which_asic + 1,uVar6), extraout_r1 == 0)) {
            putchar(10);
          }
        }
      }
  ...
  if ((Local_Config_Information->Test_Info).Display.Display_result_Every_Core != false) {
    ...
    for (which_asic = 0; which_asic < uVar2; which_asic = which_asic + 1) {
      if (gHistory_Result[gPattern_test_counter].every_asic_got_nonce_num[which_asic] <
          uVar3 * uVar4) {
        printf("asic[%03d] = %d\n",which_asic,
               gHistory_Result[gPattern_test_counter].every_asic_got_nonce_num[which_asic]);
        ...
        for (which_core = 0; which_core < statistics_core_num; which_core = which_core + 1) {
          ...
          if ((((iVar1 != 0) || (which_core != 0x23)) &&
              ((iVar1 = strcmp((Local_Config_Information->Hash_Board).Asic_Type,"BM1362"),
               iVar1 != 0 || ((which_core < 0x25 || (0x27 < which_core)))))) &&
             (gHistory_Result[gPattern_test_counter].asic_core_nonce_num[which_asic][which_core] <
              uVar4)) {
            strcmp(Local_Config_Information->Test_Process,"PT1+");
            printf("core[%03d] = %d,      lost these nonce : ",which_core,
                   gHistory_Result[gPattern_test_counter].asic_core_nonce_num[which_asic]
                   [which_core]);
  ...
  log_to_file(tmp1,time_stamp);
  avg_nonce_rate = (Local_Config_Information->Test_Info).Test_Standard.Nonce_Rate;
  if ((gTest_loop - 1 <= (uint)gPattern_test_counter) &&
     (gHistory_Result[gPattern_test_counter].frequence == 0x19f)) {
    avg_nonce_rate = avg_nonce_rate - 0x19;
  }
  if ((double)avg_nonce_rate <= gHistory_Result[gPattern_test_counter].nonce_rate * 10000.0) {
