          ...
          if (3 < log_level) {
            ...
            if (pFVar1 != (FILE *)0x0) {
              fprintf(pFVar1,"%s:%d:%s: increase volt for max chip temp is %d\n","driver-btm-soc.c",
                      0xdeb,"decrease_voltage_at_high_temp",last_testpatten_highest_chip_temp);
            }
            fclose(pFVar1);
          }
          for (chain = '\0'; chain < 0x10; chain = chain + '\x01') {
            if (dev->chain_exist[chain] != 0) {
              dVar2 = get_working_voltage_by_chain(chain);
              set_working_voltage_by_chain(chain,dVar2 + 0.2);
            }
          }
        ...
        if (3 < log_level) {
          ...
          if (pFVar1 != (FILE *)0x0) {
            fprintf(pFVar1,"%s:%d:%s: decrease volt for max chip temp is %d\n","driver-btm-soc.c",
                    0xddf,"decrease_voltage_at_high_temp",last_testpatten_highest_chip_temp);
          }
          fclose(pFVar1);
        }
        for (chain = '\0'; chain < 0x10; chain = chain + '\x01') {
