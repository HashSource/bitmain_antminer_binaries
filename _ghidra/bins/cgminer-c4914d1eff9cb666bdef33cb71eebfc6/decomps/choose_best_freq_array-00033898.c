...

{
  ...
  else if (calc_ability[scanfreq_best_times] < calc_ability[uVar2]) {
    if (3 < log_level) {
    ...
    if (3 < log_level) {
      ...
      if (pFVar1 != (FILE *)0x0) {
        fprintf(pFVar1,"%5.3f [%d times]replace %5.3f [%d times]\n",
                SUB84((double)(calc_ability[uVar2] / 1000.0),0),
                (int)((ulonglong)(double)(calc_ability[uVar2] / 1000.0) >> 0x20),freq_scan_times);
      }
      fclose(pFVar1);
    }
