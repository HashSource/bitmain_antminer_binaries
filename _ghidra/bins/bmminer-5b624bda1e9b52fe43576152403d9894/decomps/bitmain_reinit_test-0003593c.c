  ...
  *axi_fpga_addr = 0x80000000;
  ...
  do {
    while (dev->chain_exist[iVar14] == 1) {
      ...
      iVar14 = iVar13;
      if (iVar13 == 0x10) goto LAB_000359f2;
    }
    iVar14 = iVar14 + 1;
  } while (iVar14 != 0x10);
LAB_000359f2:
  ...
  do {
    while (dev->chain_exist[iVar14] == 1) {
      ...
      if (iVar13 < 0xe) {
        ...
        if (2 < iVar14 - 1U) break;
        set_voltage_T9_18_into_PIC(chain,'\x01');
      }
      iVar14 = iVar14 + 1;
      if (iVar14 == 0x10) goto LAB_00035a4e;
    }
    iVar14 = iVar14 + 1;
  } while (iVar14 != 0x10);
LAB_00035a4e:
  ...
  do {
    while (paVar12->chain_exist[iVar14] == 1) {
      ...
      sprintf(logstr,"Chain[J%d] working voltage=%d value=%d\n",iVar13,
              (uint)chain_voltage_pic[iVar14],iVar15);
      writeInitLogFile(logstr);
      iVar6 = getChainPICMagicNumber(iVar14);
      if ((iVar6 == 0x7d) &&
         (iVar6 = chain_voltage_value[iVar14],
         iVar6 != iVar15 && iVar6 + iVar2 * -10 < 0 == SBORROW4(iVar6,iVar15))) {
        dVar16 = 364.0704 / (((double)(longlong)iVar6 * 4.75) / 100.0 - 32.79) - 30.72;
      ...
      iVar13 = iVar13 + 1;
      paVar12 = dev;
      if (iVar14 == 0x10) goto LAB_00035afa;
    }
    iVar14 = iVar14 + 1;
    iVar13 = iVar13 + 1;
  } while (iVar14 != 0x10);
LAB_00035afa:
  ...
  while( true ) {
    ...
    paVar12 = dev;
    if (puVar3[iVar14] == 1) {
      set_pic_voltage((uchar)iVar14,'\0');
  ...
  do {
    while( true ) {
      iVar13 = iVar14 + 1;
      if (dev->chain_exist[iVar14 + -1] != 1) break;
      sprintf(logstr,"Chain[J%d] has %d asic\n",iVar14,
              (uint)*(byte *)((int)dev->temp + iVar14 + 0x3f));
      writeInitLogFile(logstr);
      if (*(char *)((int)dev->temp + iVar14 + 0x3f) == '\0') {
        dev->chain_exist[iVar14 + -1] = 0;
      }
      sprintf(logstr,"retry Chain[J%d] has %d asic\n",iVar14);
      writeInitLogFile(logstr);
      iVar14 = iVar13;
      if (iVar13 == 0x11) goto LAB_00035c26;
    }
    iVar14 = iVar13;
  } while (iVar13 != 0x11);
LAB_00035c26:
  ...
  do {
    while ((paVar12->chain_exist[iVar14] == 1 && (paVar12->chain_asic_num[iVar14] == '\x12'))) {
      ...
      cgsleep_ms(10);
      paVar12 = dev;
      if (iVar14 == 0x10) goto LAB_00035d22;
    }
    iVar14 = iVar14 + 1;
  } while (iVar14 != 0x10);
LAB_00035d22:
  ...
  do {
    if ((paVar11->chain_exist[0] == 1) && (paVar12->chain_asic_num[uVar8] == '\x12')) {
      if (iVar14 < 0xe) {
        iVar13 = (int)uVar8 / 3;
        if ((int)uVar8 % 3 != 1) {
          paVar7->chain_asic_temp_num[0] = paVar12->chain_asic_temp_num[iVar13 * 3 + 1];
    ...
    paVar7 = (all_parameters *)((int)&paVar7->current_job_start_address + 1);
  ...
  paVar12 = dev;
  do {
    if (paVar12->chain_exist[iVar14] == 1) {
      open_core_one_chain(iVar14,true);
      sleep(1);
      _Var5 = SUB41(iVar14,0);
      if (fpga_version < 0xe) {
        if (iVar14 % 3 == 2) {
          pthread_mutex_lock((pthread_mutex_t *)&iic_mutex);
          set_pic_voltage(_Var5,chain_voltage_pic[iVar14]);
          pthread_mutex_unlock((pthread_mutex_t *)&iic_mutex);
        }
        ...
        if (iVar14 == 1) {
          open_core_one_chain(8,_Var5);
          sleep(1);
          open_core_one_chain(9,_Var5);
          sleep(1);
        }
        else if (iVar14 == 2) {
