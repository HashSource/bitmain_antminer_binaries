...

{
  ...
  if (iVar2 == 0) {
    ...
    printf("%s ",time_stamp);
    printf("%s : %s: %s\n","create_connection_to_server_by_name",&DAT_001e8424,ip);
    snprintf(tmp1,0x100,"%s: %s",&DAT_001e8424,ip);
    ...
    for (i = '\0'; mes_host->h_addr_list[i] != (char *)0x0; i = i + '\x01') {
      __src = inet_ntoa((in_addr)*(in_addr_t *)mes_host->h_addr_list[i]);
      __s = inet_ntoa((in_addr)*(in_addr_t *)mes_host->h_addr_list[i]);
      __n = strlen(__s);
      memcpy(ip,__src,__n);
    }
    i = '\0';
    while ((i < 3 && (socket_id = socket(mes_host->h_addrtype,1,0), socket_id < 0))) {
      print_time_stamp(time_stamp,0x30);
      printf("%s ",time_stamp);
    ...
    print_time_stamp(time_stamp,0x30);
    printf("%s ",time_stamp);
    printf("%s : Can\'t get %s ip\n","create_connection_to_server_by_name",&DAT_001e8424);
    snprintf(tmp1,0x100,"Can\'t get %s ip",&DAT_001e8424);
    log_to_file(tmp1,time_stamp);
  }
  return -1;
}
