...

{
  ...
  if (conf == (void *)0x0) {
    ...
    ctx->patter_num = *(int *)((int)conf + 0x118);
    ctx->abnormal_core = 0xf;
    ctx->interval = gAddress_interval;
    (ctx->reg).ccdly_sel = *(uint32_t *)((int)conf + 0x4f8);
    (ctx->reg).pwth_sel = *(uint32_t *)((int)conf + 0x4fc);
    (ctx->reg).swpf_mode = *(uint32_t *)((int)conf + 0x500);
    (ctx->reg).pulse_mode = *(uint32_t *)((int)conf + 0x504);
    (ctx->reg).clk_sel = *(uint32_t *)((int)conf + 0x508);
    (ctx->reg).ro_relay_en = *(uint32_t *)((int)conf + 0x50c);
    (ctx->reg).co_relay_en = *(uint32_t *)((int)conf + 0x510);
    (ctx->reg).diode_vdd_mux_sel = *(uint32_t *)((int)conf + 0x514);
    (ctx->speed).baudrate = *(uint32_t *)((int)conf + 0x4f0);
    (ctx->speed).timeout = *(uint32_t *)((int)conf + 0x4f4);
    memcpy(&ctx->board,(void *)((int)conf + 0x30),0x10);
    (ctx->board).sensor.sensor_num = 4;
    ...
    (ctx->board).sensor.sensor_addr[2] = *(uint32_t *)((int)conf + 0x94);
    (ctx->board).sensor.sensor_addr[3] = *(uint32_t *)((int)conf + 0x98);
    (ctx->temp).cool_board_time = *(uint32_t *)((int)conf + 0x520);
    (ctx->temp).temp_gap = *(uint32_t *)((int)conf + 0x518);
    (ctx->temp).wait_cool_time = *(uint32_t *)((int)conf + 0x51c);
    (ctx->fan).fan_speed = *(int *)((int)conf + 0x528);
    (ctx->fan).work_fan_speed = *(int *)((int)conf + 0x52c);
    (ctx->standard).asic_nonce_rate = *(int *)((int)conf + 300);
    (ctx->standard).invalid_core_num = *(int *)((int)conf + 0x11c);
    (ctx->standard).least_nonce_per_core = *(int *)((int)conf + 0x120);
    (ctx->standard).nonce_rate = *(int *)((int)conf + 0x124);
    ctx->freq_flex = *(int *)((int)conf + 0x538);
    print_time_stamp(time_stamp,0x30);
    printf("%s ",time_stamp);
    printf("%s : freq_flex:%d\n","get_conf",ctx->freq_flex);
    snprintf(tmp1,0x100,"freq_flex:%d",ctx->freq_flex);
    log_to_file(tmp1,time_stamp);
    sweep_info->reuse = (uint)*(byte *)((int)conf + 0x104);
    sweep_info->bias = (float)(longlong)*(int *)((int)conf + 0x108) / 100.0;
    iVar2 = *(int *)((int)conf + 0x100);
    bVar3 = iVar2 == 0;
    if (bVar3) {
      iVar2 = 1;
    }
    ...
    if ((sweep_info->reuse == 0) || (re_write_sweep_cfg)) {
      sweep_info->voltage = *(uint32_t *)((int)conf + 0x138);
      sweep_info->range_freq[0] = (float)*(uint *)((int)conf + (test_levels + 0x12) * 0x10 + 0xc);
      sweep_info->range_freq[1] =
           (float)(longlong)(int)(sweep_info->range_freq[0] * (sweep_info->bias + 1.0));
    }
    iVar2 = strncmp((char *)conf,"PT3",3);
    if ((iVar2 == 0) || (*(char *)((int)conf + 0x105) != '\0')) {
      if (pt2_level < 0) {
        sweep_info->range_freq[0] =
             (float)((*(int *)((int)conf + (test_levels + 0x12) * 0x10 + 0xc) + ctx->freq_flex) -
                    0x19);
        print_time_stamp(time_stamp,0x30);
        printf("%s ",time_stamp);
        printf("%s : pt2 test fail at last level,sweep level adjust from [%d]-->[%d]\n","get_conf",
               *(undefined4 *)((int)conf + (test_levels + 0x12) * 0x10 + 0xc),
               *(int *)((int)conf + (test_levels + 0x12) * 0x10 + 0xc) + -0x19);
        ...
        sweep_info->range_freq[0] =
             (float)(uint)(*(int *)((int)conf + (pt2_level + 0x13) * 0x10 + 0xc) + ctx->freq_flex);
      }
      else {
        print_time_stamp(time_stamp,0x30);
        printf("%s ",time_stamp);
    ...
    sweep_info->pre_open_core_voltage = *(uint32_t *)((int)conf + (test_levels + 3) * 0x10 + 0xf4);
    if (*(char *)((int)conf + 0x10d) != '\0') {
      sweep_info->range_freq[0] = (float)(longlong)*(int *)((int)conf + 0x110);
      sweep_info->range_freq[1] = (float)(longlong)*(int *)((int)conf + 0x114);
      print_time_stamp(time_stamp,0x30);
      printf("%s ",time_stamp);
    ...
    for (i = 0; i < test_levels; i = i + 1) {
      ctx->tests[i].voltage = *(int *)((int)conf + (i + 0x13) * 0x10 + 8);
      ctx->tests[i].pre_open_core_voltage = *(int *)((int)conf + (i + 4) * 0x10 + 0xf4);
      ctx->tests[i].level = *(int *)((int)conf + (i + 4) * 0x10 + 0xf0);
      ctx->tests[i].freq = *(int *)((int)conf + (i + 0x13) * 0x10 + 0xc);
      print_time_stamp(time_stamp,0x30);
      printf("%s ",time_stamp);
