...

{
  ...
  *pp_Var2 = p_Var3 + -1;
  ...
  OPENSSL_sk_pop_free(ssl->cert,0xdb4d1);
  ssl->cert = (cert_st *)0x0;
  OPENSSL_sk_pop_free(ssl->sid_ctx_length,(undefined *)0x16a341);
  ...
  OPENSSL_sk_pop_free(ssl[3].expand,0x1691dd);
  OPENSSL_sk_pop_free(ssl[3].read_hash,0x13f3f5);
  ...
  OPENSSL_sk_pop_free(ssl[3].rwstate,0x169841);
  OPENSSL_sk_pop_free(ssl[3].in_handshake,0x169841);
  OPENSSL_sk_pop_free(ssl[3].rbio,(undefined *)0x16a341);
  if (ssl->type != 0) {
