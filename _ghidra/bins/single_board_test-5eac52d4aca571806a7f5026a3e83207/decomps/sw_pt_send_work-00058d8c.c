...

{
  ...
  uVar4 = *(uint *)(*(int *)((int)args + 0x218c4) + 4);
  uVar5 = (g_rt.config)->big_core * (g_rt.config)->samll_core_num_in_big_core;
  uVar6 = *(uint *)(*(int *)((int)args + 0x218c4) + 0x1c);
  uVar7 = *(uint *)(*(int *)((int)args + 0x218c4) + 0x20);
  ...
  for (which_asic = 0; which_asic < uVar4; which_asic = which_asic + 1) {
    iVar8 = *(int *)((int)args + (which_asic + 0x85f0) * 4);
    for (which_core = 0; which_core < uVar5; which_core = uVar6 + which_core) {
      for (which_pattern = 0; which_pattern < uVar7; which_pattern = which_pattern + 1) {
        while (uVar1 = is_work_fifo_ready(**(uint32_t **)((int)args + 0x218c4)), uVar1 == '\0') {
          usleep(100);
        }
        memset(&work_buf,0,0x214);
        work_buf.work_type = '\x01';
        work_buf.chain_id = (byte)**(undefined4 **)((int)args + 0x218c4) | 0x80;
        ...
        ;
        work = (pattern_work *)((which_pattern + uVar7 * which_core) * 0x10 + iVar8);
        for (i = 0; i < uVar6; i = i + 1) {
          if (((which_core - 0xf0 < 0x10) || (which_core - 0x7f0 < 0x10)) && (0xb < i)) {
            ...
            iVar2 = __aeabi_uidiv(which_core,*(undefined4 *)(*(int *)((int)args + 0x218c4) + 0x1c));
            work = (pattern_work *)
                   ((which_pattern +
                    uVar7 * (i + *(int *)(*(int *)((int)args + 0x218c4) + 0x1c) * iVar2)) * 0x10 +
                   iVar8);
            for (j = 0; j < 0x20; j = j + 1) {
              if (j < 0x18) {
                work_buf.midstate[i][j] = '\0';
            ...
            *(int *)((int)args + 0x218f8) = *(int *)((int)args + 0x218f8) + 1;
          }
        }
  ...
  if (*(uint *)((int)args + 0x218f8) < uVar4 * uVar7 * (g_rt.config)->asic_small_cores) {
    print_time_stamp(time_stamp,0x30);
    printf("%s ",time_stamp);
    printf("%s : Send work num : %d, less than %d\n","sw_pt_send_work",
           *(undefined4 *)((int)args + 0x218f8),uVar4 * uVar5 * uVar7);
    snprintf(tmp1,0x100,"Send work num : %d, less than %d",*(undefined4 *)((int)args + 0x218f8),
             uVar4 * uVar5 * uVar7);
    log_to_file(tmp1,time_stamp);
  }
  else {
    print_time_stamp(time_stamp,0x30);
    printf("%s ",time_stamp);
    printf("%s : Send test %d pattern done\n","sw_pt_send_work",*(undefined4 *)((int)args + 0x218f8)
          );
    snprintf(tmp1,0x100,"Send test %d pattern done",*(undefined4 *)((int)args + 0x218f8));
    log_to_file(tmp1,time_stamp);
  }
  uVar9 = *(undefined4 *)((int)args + 0x216e8);
  print_time_stamp(time_stamp,0x30);
  printf("%s ",time_stamp);
  printf("%s : send work thread eixt,send:%d,recv:%d, repeat:%d\n","sw_pt_send_work",
         *(undefined4 *)((int)args + 0x218f8),uVar9,*(undefined4 *)((int)args + 0xc34));
