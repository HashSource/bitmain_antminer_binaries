...

{
  ...
  for (i = '\0'; (uint)i < (uint)get_local_ip_timeout_counter; i = i + '\x01') {
    ...
    if (iVar2 == 0) {
      print_time_stamp(time_stamp,0x30);
      printf("%s ",time_stamp);
      printf("%s : %s: %s\n","create_connection_to_MES_system_by_name",&DAT_001e9bf8,ip);
      snprintf(tmp1,0x100,"%s: %s",&DAT_001e9bf8,ip);
      log_to_file(tmp1,time_stamp);
      break;
    }
    if (get_local_ip_timeout_counter + -1 <= (int)(uint)i) {
      ...
      print_time_stamp(time_stamp,0x30);
      printf("%s ",time_stamp);
      printf("%s : Can\'t get %s ip\n","create_connection_to_MES_system_by_name",&DAT_001e9bf8);
      snprintf(tmp1,0x100,"Can\'t get %s ip",&DAT_001e9bf8);
      log_to_file(tmp1,time_stamp);
      return false;
    }
  }
  ...
  while (((uint)i < (uint)get_host_timeout_counter &&
    ...
    snprintf(tmp1,0x100,"try get MES system ip: %d",(uint)i);
    log_to_file(tmp1,time_stamp);
    sleep(3);
  ...
  snprintf(tmp1,0x100,"Address type: %s",pcVar4);
  log_to_file(tmp1,time_stamp);
  for (i = '\0'; mes_host->h_addr_list[i] != (char *)0x0; i = i + '\x01') {
    ...
    pcVar4 = inet_ntoa((in_addr)*(in_addr_t *)mes_host->h_addr_list[i]);
    snprintf(tmp1,0x100,"IP addr %d: %s",i + 1,pcVar4);
    log_to_file(tmp1,time_stamp);
    pcVar4 = inet_ntoa((in_addr)*(in_addr_t *)mes_host->h_addr_list[i]);
    ...
    snprintf(tmp1,0x100,"ip addr: %s",ip);
    log_to_file(tmp1,time_stamp);
  }
  i = '\0';
  do {
...
LAB_0003cc72:
      ...
      while( true ) {
        ...
        snprintf(tmp1,0x100,"timeout_counter = %d",connect_MES_system_timeout_counter);
        log_to_file(tmp1,time_stamp);
      }
