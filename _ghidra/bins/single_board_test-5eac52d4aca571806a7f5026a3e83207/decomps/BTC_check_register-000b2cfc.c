...

{
  ...
  if (reg_value_buf->reg_value_num < 0xfff) {
    ...
    for (i = 0; i < 10; i = i + 1) {
      ...
      printf("%s ",time_stamp);
      uVar4 = *(undefined4 *)
               ("IPAddressRange_it" + (int)((int)&reg_value_buf->reg_buffer[i].chain_number + 1));
      uVar1 = __aeabi_uidiv(*(undefined4 *)
                             ("X509_ATTRIBUTE_free" +
                             (int)((int)&reg_value_buf->reg_buffer[i].reg_value + 3)),
                            gAddress_interval);
      printf("%s : reg_value = 0x%08x, which_asic = %d, reg_address = 0x%08x\n","BTC_check_register"
             ,uVar4,uVar1,
             *(undefined4 *)
              ("X509_ATTRIBUTE_free" + (int)((int)&reg_value_buf->reg_buffer[i].chip_address + 3)));
      uVar4 = *(undefined4 *)
               ("IPAddressRange_it" + (int)((int)&reg_value_buf->reg_buffer[i].chain_number + 1));
      uVar1 = __aeabi_uidiv(*(undefined4 *)
                             ("X509_ATTRIBUTE_free" +
