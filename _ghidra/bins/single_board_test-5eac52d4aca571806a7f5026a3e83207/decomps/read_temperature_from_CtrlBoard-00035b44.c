...

{
  ...
  while( true ) {
    ...
    for (i = '\0'; i < gCtrlBoard_sensor_num; i = i + '\x01') {
      memset(data,-1,2);
      iVar4 = pca_switch_read((uint8_t)gChain,i,'p','L',data,data + 1);
      if (iVar4 == 0) {
        ...
        print_time_stamp(time_stamp,0x30);
        printf("%s ",time_stamp);
        printf("%s : Read switch sensor fail,addr: %d\n","read_temperature_from_CtrlBoard",0xe0);
        snprintf(tmp1,0x100,"Read switch sensor fail,addr: %d",0xe0);
        log_to_file(tmp1,time_stamp);
        gHistory_Result[gPattern_test_counter].ctrlboard_sensor_OK[i] = false;
        read_fail_cnt = read_fail_cnt + 1;
      }
    ...
    if (!_Var3) {
      ...
      printf("%s : WARNING temp[0]:%d,temp[1]:%d\n","read_temperature_from_CtrlBoard",
             (int)gHistory_Result[gPattern_test_counter].sensor_local_temperature_from_ctrlboard[0],
             (int)gHistory_Result[gPattern_test_counter].sensor_local_temperature_from_ctrlboard[1])
      ;
