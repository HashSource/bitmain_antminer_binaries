...

{
  ...
  do {
    ...
    usleep(60000);
    get_register_value_with_ext_data(which_asic_address,0xbd,true,"BM1370",&adc_voltage);
    if ((int)adc_voltage < 0) {
      vol = adc_voltage & 0x7fff;
      if (loop != 0) {
        vol_sum = vol + vol_sum;
      }
      loop = loop + 1;
    ...
    loop_num = loop_num + 1;
  } while ((loop != max_loop) && (loop_num <= end_loop));
  if (1 < loop) {
