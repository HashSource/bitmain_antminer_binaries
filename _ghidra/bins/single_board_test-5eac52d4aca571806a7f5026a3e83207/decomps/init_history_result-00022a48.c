...

{
  ...
  if (test_loop_counter == 0) {
    memset(gHistory_Result,0,0x7a97c0);
  }
  ...
  memset(gHistory_Result[test_loop_counter].ok_asic_list,0,0x41);
  memset(gHistory_Result[test_loop_counter].bad_asic_list,1,0x41);
  memset(gHistory_Result[test_loop_counter].pic_sensor_OK,0,4);
  memset(gHistory_Result[test_loop_counter].asic_sensor_OK,1,0x41);
