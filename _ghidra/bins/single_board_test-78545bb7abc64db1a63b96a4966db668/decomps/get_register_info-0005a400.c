...

{
  ...
  case 0xe:
    ...
    break;
  case 0xf:
    send_get_status_command((uint8_t)(g_rt.config)->chain,'\x01',0,0xbd);
  }
  bad_asic_number = 0;
  do {
    while( true ) {
      ...
      print_time_stamp(time_stamp,0x30);
      printf("%s ",time_stamp);
      printf("%s : reg_value_num: %d\n","get_register_info",uVar4);
      snprintf(tmp1,0x100,"reg_value_num: %d",uVar4);
      log_to_file(tmp1,time_stamp);
    ...
    for (i = 0; i < uVar4; i = i + 1) {
      ...
      case 0:
        if (register_value >> 0x10 == (uint)asic_name) {
          ...
          iVar3 = __aeabi_idiv(uVar1 & 0xff,(g_rt.config)->interval);
          g_rt.asic_stats.ok_asic_list[iVar3] = true;
        }
        else {
      ...
      case 5:
        ...
        g_rt.asic_stats.every_asic_got_nonce_num[iVar3] =
      ...
      case 7:
        iVar3 = __aeabi_uidiv(reg_value_buf->reg_buffer[reg_value_buf->p_rd].chip_address,
                              (g_rt.config)->interval);
        g_rt.asic_stats.every_asic_reg_nonce_error_num[iVar3] = uVar1;
        break;
      case 10:
        print_time_stamp(time_stamp,0x30);
        printf("%s ",time_stamp);
      ...
      case 0xc:
        iVar3 = __aeabi_uidiv(reg_value_buf->reg_buffer[reg_value_buf->p_rd].chip_address,
                              (g_rt.config)->interval);
        g_rt.asic_stats.every_asic_got_nonce_num[iVar3] = uVar1;
        g_rt.asic_stats.valid_nonce_num = g_rt.asic_stats.valid_nonce_num + register_value;
        break;
      case 0xd:
        iVar3 = __aeabi_uidiv(reg_value_buf->reg_buffer[reg_value_buf->p_rd].chip_address,
                              (g_rt.config)->interval);
        g_rt.asic_stats.every_asic_reg_got_nonce_num[iVar3] = uVar1;
        break;
      case 0xe:
        iVar3 = __aeabi_uidiv(reg_value_buf->reg_buffer[reg_value_buf->p_rd].chip_address,
                              (g_rt.config)->interval);
        g_rt.asic_stats.every_asic_reg_ntick_nonce_num[iVar3] = uVar1;
        break;
      case 0xf:
        if (mode) {
          if (reg_value_buf->reg_buffer[reg_value_buf->p_rd].reg_address == 0xbd) {
            uVar5 = reg_value_buf->reg_buffer[reg_value_buf->p_rd].reg_value;
            uVar6 = reg_value_buf->reg_buffer[reg_value_buf->p_rd].chain_number;
            bVar2 = __aeabi_uidiv(reg_value_buf->reg_buffer[reg_value_buf->p_rd].chip_address,
                                  (g_rt.config)->interval);
            if ((int)uVar5 < 0) {
              iVar3 = g_chain_vol_data[uVar6 & 0xff].asic_vol_data_info[bVar2].
                      asic_returned_vol_count;
              g_chain_vol_data[uVar6 & 0xff].asic_vol_data_info[bVar2].asic_returned_vol_count =
                   iVar3 + 1;
              if (0 < iVar3) {
                g_chain_vol_data[uVar6 & 0xff].asic_vol_data_info[bVar2].asic_returned_vol_data =
                     (uVar5 & 0x7fff) +
                     g_chain_vol_data[uVar6 & 0xff].asic_vol_data_info[bVar2].asic_returned_vol_data
                ;
              }
              if (g_chain_vol_data[uVar6 & 0xff].got_vol_flag_everytime[bVar2] != true) {
                g_chain_vol_data[uVar6 & 0xff].got_vol_flag_everytime[bVar2] = true;
                g_chain_vol_data[uVar6 & 0xff].got_vol_asics_everytime =
                     g_chain_vol_data[uVar6 & 0xff].got_vol_asics_everytime + 1;
              }
              if ((8 < g_chain_vol_data[uVar6 & 0xff].asic_vol_data_info[bVar2].
                       asic_returned_vol_count) &&
                 (g_chain_vol_data[uVar6 & 0xff].asic_vol_data_info[bVar2].asic_finish_vol_data_flag
                  != true)) {
                g_chain_vol_data[uVar6 & 0xff].asic_vol_data_info[bVar2].asic_finish_vol_data_flag =
                     true;
                g_chain_vol_data[uVar6 & 0xff].finish_read_vol_asics =
                     g_chain_vol_data[uVar6 & 0xff].finish_read_vol_asics + 1;
              }
            }
          }
          else {
            print_time_stamp(time_stamp,0x30);
            printf("%s ",time_stamp);
            printf("%s : back reg address : 0x%x, need reg address : 0x%x\n","get_register_info",
                   register_value,0xbd);
            snprintf(tmp1,0x100,"back reg address : 0x%x, need reg address : 0x%x",register_value,
                     0xbd);
            log_to_file(tmp1,time_stamp);
          }
        }
