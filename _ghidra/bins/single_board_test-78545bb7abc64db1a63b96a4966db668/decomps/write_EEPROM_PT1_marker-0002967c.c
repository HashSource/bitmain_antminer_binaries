...

{
  ...
  if (gHistory_Result[gPattern_test_counter].eeprom_ok == false) {
    ...
    snprintf(tmp1,0x100,"gEEPROM_OK is %s, so no need write eeprom PT1 marker",pcVar2);
      ...
      if (iVar1 == 0) {
        if (marker_back == 'Z') {
          ...
          printf("%s : check EEPROM marker fail. read bak is: 0x%02x\n","write_EEPROM_PT1_marker",
                 (uint)marker_back);
          snprintf(tmp1,0x100,"check EEPROM marker fail. read bak is: 0x%02x",(uint)marker_back);
          log_to_file(tmp1,time_stamp);
