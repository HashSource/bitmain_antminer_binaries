...

{
  ...
  if (iVar2 == 6) {
    ...
    usleep(300000);
    if ((read_back_data[0] == 6) && (read_back_data[1] == 1)) goto LAB_000d24de;
    snprintf(tmp42,0x800,"%s failed! read_back_data[0] = 0x%02x, read_back_data[1] = 0x%02x\n\n",
             "_bitmain_pic_start_app_common",(uint)read_back_data[0],(uint)read_back_data[1]);
    _applog(0,tmp42,false);
  }
  else {
    __format = "%s write iic err\n";
LAB_000d2528:
    snprintf(tmp42,0x800,__format,"_bitmain_pic_start_app_common");
    _applog(0,tmp42,false);
  }
  iVar3 = -0x7ffffe00;
LAB_000d24de:
