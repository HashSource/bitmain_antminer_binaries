...

{
  ...
  if (_Var1) {
    if (m_info.data == (char *)0x0) {
      ...
      uVar6 = (rawtime >> 0x1f) << 2 | (uint)rawtime >> 0x1e;
      snprintf(nonce,0x14,"%llu",rawtime * 1000,rawtime * 1000,
               ((((uVar6 << 5 | (uint)(rawtime * 4) >> 0x1b) - uVar6) -
                (uint)((uint)(rawtime * 0x80) < (uint)(rawtime * 4))) +
               (rawtime >> 0x1f) + (uint)CARRY4(rawtime * 0x7c,rawtime)) * 8 |
