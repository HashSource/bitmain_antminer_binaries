    ...
    if (socket_id == -1) {
      ...
      if (_Var2) {
        do {
          ...
          iVar3 = socket_id + 0x1f;
          if (-1 < socket_id) {
            iVar3 = socket_id;
          }
          *(uint *)(time_stamp + (iVar3 >> 5) * 4) =
               *(uint *)(time_stamp + (iVar3 >> 5) * 4) | 1 << (socket_id % 0x20 & 0xffU);
          iVar3 = select(socket_id + 1,(fd_set *)0x0,(fd_set *)time_stamp,(fd_set *)0x0,
                         (timeval *)&tv_timeout);
          if (0 < iVar3) {
            iVar4 = socket_id + 0x1f;
            if (-1 < socket_id) {
              iVar4 = socket_id;
            }
            if ((*(int *)(time_stamp + (iVar4 >> 5) * 4) >> (socket_id % 0x20 & 0xffU) & 1U) != 0) {
              len = 4;
