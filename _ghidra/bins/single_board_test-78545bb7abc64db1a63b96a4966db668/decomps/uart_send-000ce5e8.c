    ...
    if (size <= max_fpga_can_send_uart_data_len) {
      ...
      do {
        ...
        if (-1 < (int)max_fpga_can_send_uart_data_len) {
          ...
          if (uVar3 != 0) {
            ...
            do {
              ...
              fpga_write(chain_send_buffer_addr,
                         (uint)puVar4[1] << 0x10 | (uint)send_buf[iVar2] << 0x18 | (uint)puVar4[3] |
                         (uint)puVar4[2] << 8);
              puVar4 = puVar4 + 4;
            } while (uVar3 != uVar5);
          }
          ...
          if (uVar5 != 0) {
            if (uVar5 == 2) {
              val = (uint)send_buf[uVar3 * 4] << 0x18 | (uint)send_buf[uVar3 * 4 + 1] << 0x10;
            }
            else if (uVar5 == 3) {
              val = (uint)send_buf[uVar3 * 4 + 2] << 8 | (uint)send_buf[uVar3 * 4 + 1] << 0x10 |
                    (uint)send_buf[uVar3 * 4] << 0x18;
            }
            else {
              val = (uint)send_buf[uVar3 * 4] << 0x18;
            }
