...

{
  int32_t iVar1;
  _Bool _Var2;
  uint8_t auStack_7a0 [512];
  byte abStack_5a0 [512];
  byte abStack_3a0 [496];
  ...
  if (iVar1 < 0) {
    ...
    if ((Local_Config_Information->Repair_Mode).Clear_EEPROM_Data == false) {
      ...
      log_to_file(tmp1,time_stamp);
      iVar1 = eeprom_read(gChain,'\0',auStack_7a0,EEPROM_length_local);
      if (iVar1 == 0) {
        iVar1 = eeprom_write(gChain,'\0',abStack_3a0,EEPROM_length_local);
        if (iVar1 == 0) {
          iVar1 = eeprom_read(gChain,'\0',abStack_5a0,EEPROM_length_local);
          if (iVar1 == 0) {
            for (i = 0; i < EEPROM_length_local; i = i + 1) {
              if (abStack_3a0[i] != abStack_5a0[i]) {
                print_time_stamp(time_stamp,0x30);
                printf("%s ",time_stamp);
                printf("%s : EEPROM check fail! write_buf[%d] = 0x%02x, read_buf[%d] = 0x%02x\n",
                       "Check_EEPROM",i,(uint)abStack_3a0[i]);
                snprintf(tmp1,0x100,
            ...
            iVar1 = eeprom_write(gChain,'\0',auStack_7a0,EEPROM_length_local);
            if (iVar1 == 0) {
        ...
        if (iVar1 == 0) {
          for (i = 0; i < EEPROM_length_local; i = i + 1) {
            if (abStack_3a0[i] != abStack_5a0[i]) {
              ...
              printf("%s : EEPROM check fail! write_buf[%d] = 0x%02x, read_buf[%d] = 0x%02x\n",
                     "Check_EEPROM",i,(uint)abStack_3a0[i]);
              snprintf(tmp1,0x100,"EEPROM check fail! write_buf[%d] = 0x%02x, read_buf[%d] = 0x%02x"
                       ,i);
              log_to_file(tmp1,time_stamp);
              return false;
            }
