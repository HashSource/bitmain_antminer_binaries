...

{
  ...
  if (reg_value_buf->reg_value_num < 0xfff) {
    ...
    for (i = 0; i < 10; i = i + 1) {
      ...
      printf("%s ",time_stamp);
      uVar4 = *(undefined4 *)
               ("X509V3_conf_free" + (int)((int)&reg_value_buf->reg_buffer[i].chip_address + 1));
      uVar1 = __aeabi_uidiv(*(undefined4 *)
                             ("X509V3_conf_free" +
                             (int)((int)&reg_value_buf->reg_buffer[i].reg_address + 1)),
                            gAddress_interval);
      printf("%s : reg_value = 0x%08x, which_asic = %d, reg_address = 0x%08x\n","BTC_check_register"
             ,uVar4,uVar1,
             *(undefined4 *)("X509V3_conf_free" + (int)((int)&reg_value_buf->reg_buffer[i].crc + 1))
            );
      uVar4 = *(undefined4 *)
               ("X509V3_conf_free" + (int)((int)&reg_value_buf->reg_buffer[i].chip_address + 1));
      uVar1 = __aeabi_uidiv(*(undefined4 *)
      ...
      snprintf(tmp1,0x100,"reg_value = 0x%08x, which_asic = %d, reg_address = 0x%08x",uVar4,uVar1,
               *(undefined4 *)
                ("X509V3_conf_free" + (int)((int)&reg_value_buf->reg_buffer[i].crc + 1)));
      log_to_file(tmp1,time_stamp);
