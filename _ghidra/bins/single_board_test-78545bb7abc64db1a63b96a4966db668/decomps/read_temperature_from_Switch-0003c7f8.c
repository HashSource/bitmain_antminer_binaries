...

{
  ...
  snprintf(tmp1,0x100,"Start %s","read_temperature_from_Switch");
  ...
  while (gStart_read_temp != false) {
    for (index = 0; index < 8; index = index + 1) {
      ...
      snprintf(tmp1,0x100,"index: %d  temp :%d",index,iVar1);
      log_to_file(tmp1,time_stamp);
      if (iVar1 == 0) {
  ...
  printf("%s ",time_stamp);
  printf("%s : %s stop\n","read_temperature_from_Switch","read_temperature_from_Switch");
  snprintf(tmp1,0x100,"%s stop","read_temperature_from_Switch");
  log_to_file(tmp1,time_stamp);
  return (void *)0x0;
}
