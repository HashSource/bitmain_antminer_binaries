    ...
    if ((_g_eeprom_state[which_chain].chain_has_opened != 0) ||
      ...
      if (len != 0) {
        ...
        do {
          puVar2 = data + 1;
          tmp_addr = addr;
          iVar1 = iic_write_reg(_g_eeprom_state[which_chain].eeprom_fd,&tmp_addr,1,data,1,true);
          addr = addr + '\x01';
          if (iVar1 != 1) {
            snprintf(tmp42,0x800,"fail to write eeprom by iic, chain:%d, addr: %d",which_chain,
                     (uint)tmp_addr);
