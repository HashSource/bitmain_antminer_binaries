...
/* WARNING: Removing unreachable block (ram,0x000300ac) */
    ...
    if (which_function != 3) {
      if (3 < which_function) {
        if (which_function == 6) {
          ...
          if (which_function != 5) goto LAB_0002fb4e;
      ...
      if (which_function < 2) {
        gAsic_num = 0;
        memset(gHistory_Result[gPattern_test_counter].ok_asic_list,0,0x100);
        memset(gHistory_Result[gPattern_test_counter].bad_asic_list,1,0x100);
...
LAB_0002fb4e:
    ...
    if (which_function != 0xe) goto LAB_0002fb4e;
...
LAB_0002fbb8:
  ...
  do {
    while( true ) {
      ...
      print_time_stamp(time_stamp,0x30);
      printf("%s ",time_stamp);
      printf("%s : reg_value_num: %d\n","get_register_value_with_ext_data",uVar4);
...
LAB_000307ce:
            ...
            if (which_function_local != 5) goto LAB_000307ce;
            uVar2 = (uint)gPattern_test_counter;
            iVar3 = __aeabi_uidiv(reg_value_buf->reg_buffer[reg_value_buf->p_rd].chip_address,
                                  gAddress_interval);
          ...
          if ((int)register_value < 0) {
            ...
            if (uVar2 < (Local_Config_Information->Hash_Board).Asic_Num) {
              ...
              *(double *)((int)ext_data + (iVar3 + 3) * 8) =
              ...
              iVar3 = __aeabi_uidiv(uVar6,gAddress_interval);
              *(undefined1 *)((int)ext_data + iVar3 + 0x81c) = 1;
            }
          }
        }
          ...
          if (which_function_local != 0xe) goto LAB_000307ce;
          uVar2 = (uint)gPattern_test_counter;
          iVar3 = __aeabi_uidiv(reg_value_buf->reg_buffer[reg_value_buf->p_rd].chip_address,
                                gAddress_interval);
