...

{
  ...
  *pp_Var2 = p_Var3 + -1;
  ...
  OPENSSL_sk_pop_free(ssl->cert,0xdf721);
  ssl->cert = (cert_st *)0x0;
  OPENSSL_sk_pop_free(ssl->sid_ctx_length,(undefined *)0x16e591);
  ...
  OPENSSL_sk_pop_free(ssl[3].expand,0x16d42d);
  OPENSSL_sk_pop_free(ssl[3].read_hash,0x143645);
  ...
  OPENSSL_sk_pop_free(ssl[3].rwstate,0x16da91);
  OPENSSL_sk_pop_free(ssl[3].in_handshake,0x16da91);
  OPENSSL_sk_pop_free(ssl[3].rbio,(undefined *)0x16e591);
  if (ssl->type != 0) {
