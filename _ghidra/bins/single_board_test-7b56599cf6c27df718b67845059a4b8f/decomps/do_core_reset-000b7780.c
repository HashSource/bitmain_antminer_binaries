...

{
  ...
  snprintf(tmp1,0x100,"interval %d, total ASIC num %d",gAddress_interval,
           (Local_Config_Information->Hash_Board).Asic_Num);
  ...
  snprintf(tmp1,0x100,"ccdly pwth %d %d",
           (Local_Config_Information->Test_Info).Asic_Register.CCdly_Sel,
           (Local_Config_Information->Test_Info).Asic_Register.Pwth_Sel);
  log_to_file(tmp1,time_stamp);
  for (asic = 0; (uint)asic < (Local_Config_Information->Hash_Board).Asic_Num; asic = asic + 1) {
    set_asic_core_srst_fast((uint8_t)gChain,asic * gAddress_interval);
    usleep(10000);
    set_asic_clock_select_control
              ((uint8_t)gChain,asic * gAddress_interval,
    ...
    set_asic_core_enable((uint8_t)gChain,asic * gAddress_interval);
    usleep(10000);
  }
  return;
}
