...

{
  ...
  inc_freq_with_target(last_matrix,(g_rt.config)->asics,freq,0,state);
  usleep(100000);
  reset_bad_asic_freq(last_matrix,(g_rt.config)->asics,state);
  ...
  pause_recv_nonce();
  calc_nonce_stats(&g_rt.stats,&g_rt.asic_stats);
