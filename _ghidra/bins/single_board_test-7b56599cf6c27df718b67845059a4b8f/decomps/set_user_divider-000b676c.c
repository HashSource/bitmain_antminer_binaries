...
void set_user_divider(uint32_t chain,uint8_t mode,uint8_t chip_addr,uint8_t pll_id)

{
  ...
  if (mode == '\0') {
    ...
    set_chain_pllparameter_userdivider((uint8_t)chain,pll_id,(pll_userdivider_t)0x0);
  }
  return;
}
