...

{
  ...
  if (iVar2 == 0) {
    ...
    printf("%s ",time_stamp);
    printf("%s : %s: %s\n","create_connection_to_server_by_name",&DAT_001ec32c,ip);
    snprintf(tmp1,0x100,"%s: %s",&DAT_001ec32c,ip);
    ...
    print_time_stamp(time_stamp,0x30);
    printf("%s ",time_stamp);
    printf("%s : Can\'t get %s ip\n","create_connection_to_server_by_name",&DAT_001ec32c);
    snprintf(tmp1,0x100,"Can\'t get %s ip",&DAT_001ec32c);
    log_to_file(tmp1,time_stamp);
  }
  return -1;
}
