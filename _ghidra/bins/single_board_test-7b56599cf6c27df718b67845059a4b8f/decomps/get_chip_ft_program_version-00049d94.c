...

{
  ...
  if (qr_code->chip_ftversion[0] == 'F') {
    ...
    *ft_version = (uint8_t)iVar1;
    if (qr_code->chip_ftversion[2] == 'V') {
      ...
      if (qr_code->chip_ftversion[5] == 'B') {
        ...
        if (qr_code->chip_ftversion[7] == 'C') {
          ...
          snprintf(tmp1,0x100,"FT version: F%dV02%dB%dC%d",(uint)*ft_version,(uint)ft_version[1],
          ...
          snprintf(tmp1,0x100,"4th FT version is not C, but is %c",
                   (uint)(byte)qr_code->chip_ftversion[7]);
          log_to_file(tmp1,time_stamp);
          _Var2 = false;
        }
        ...
        snprintf(tmp1,0x100,"3rd FT version is not B, but is %c",
                 (uint)(byte)qr_code->chip_ftversion[5]);
        log_to_file(tmp1,time_stamp);
        _Var2 = false;
      }
      ...
      snprintf(tmp1,0x100,"2nd FT version is not V, but is %c",
               (uint)(byte)qr_code->chip_ftversion[2]);
      log_to_file(tmp1,time_stamp);
      _Var2 = false;
    }
    ...
    snprintf(tmp1,0x100,"1st FT version is not F, but is %c",(uint)(byte)qr_code->chip_ftversion[0])
    ;
    log_to_file(tmp1,time_stamp);
    _Var2 = false;
  }
