
int test_r4_cipher(void)

{
  int iVar1;
  int iVar2;
  undefined4 in_r3;
  
  if (r4_cipher == 0) {
    iVar1 = EVP_CIPHER_meth_new(5,1,0x10,&r4_cipher,in_r3);
    if ((((iVar1 == 0) || (iVar2 = EVP_CIPHER_meth_set_iv_length(iVar1,0), iVar2 == 0)) ||
        (iVar2 = EVP_CIPHER_meth_set_flags(iVar1,8), iVar2 == 0)) ||
       (((iVar2 = EVP_CIPHER_meth_set_init(iVar1,0x1b2e0d), iVar2 == 0 ||
         (iVar2 = EVP_CIPHER_meth_set_do_cipher(iVar1,0x1b2df1), iVar2 == 0)) ||
        (iVar2 = EVP_CIPHER_meth_set_impl_ctx_size(iVar1,0x418), iVar2 == 0)))) {
      EVP_CIPHER_meth_free(iVar1);
      iVar1 = 0;
    }
    r4_cipher = iVar1;
    return iVar1;
  }
  return r4_cipher;
}

