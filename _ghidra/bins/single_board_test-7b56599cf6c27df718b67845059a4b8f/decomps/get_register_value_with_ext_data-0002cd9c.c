...
/* WARNING: Removing unreachable block (ram,0x0002d5fc) */
...

{
  ...
  if (which_function == 7) {
    ...
    if (which_function != 3) {
      if (3 < which_function) {
        if (which_function == 5) {
          send_get_status_command(which_chain,'\x01',0,0xe0);
        }
        else if (which_function < 6) {
          gHardware_sweep_done_asic_num = 0;
          check_chain_hwsweep_done(which_chain);
        }
        else {
      ...
      if (which_function < 2) {
        gAsic_num = 0;
        memset(gHistory_Result[gPattern_test_counter].ok_asic_list,0,0x6c);
        memset(gHistory_Result[gPattern_test_counter].bad_asic_list,1,0x6c);
    ...
    if (which_function == 9) {
      send_get_status_command(which_chain,'\0',which_asic * gAddress_interval,0x1c);
    }
...
LAB_0002d0f4:
  ...
  else if (which_function == 0xb4) {
    send_get_status_command(which_chain,'\x01',0,0xb4);
  }
  else {
    if (which_function != 0xbd) goto LAB_0002d0f4;
...
LAB_0002dd34:
            ...
            if (which_function_local != 5) goto LAB_0002dd34;
            uVar2 = (uint)gPattern_test_counter;
            iVar3 = __aeabi_uidiv(reg_value_buf->reg_buffer[reg_value_buf->p_rd].chip_address,
                                  gAddress_interval);
        ...
        else if (which_function_local == 0xb4) {
          if (uVar5 == 0xb4) {
              ...
              if (uVar2 < (Local_Config_Information->Hash_Board).Asic_Num) {
                ...
                *(double *)((iVar3 + 3) * 8 + (int)ext_data) =
                     (double)(float)((double)(longlong)(int)(register_value & 0xffff) * 0.171342 -
                                    299.5144);
                iVar3 = __aeabi_uidiv(uVar6,gAddress_interval);
                *(undefined1 *)((int)ext_data + iVar3 + 0x37c) = 1;
              }
            }
            else {
              print_time_stamp(time_stamp,0x30);
            ...
            printf("%s : read register addr err,resp:%d,expect:%d,asic_addr:%d\n",
                   "get_register_value_with_ext_data",uVar5,0xb4,uVar6);
            snprintf(tmp1,0x100,"read register addr err,resp:%d,expect:%d,asic_addr:%d",uVar5,0xb4,
                     uVar6);
            log_to_file(tmp1,time_stamp);
          }
        }
          ...
          if ((int)register_value < 0) {
            ...
            if (uVar2 < (Local_Config_Information->Hash_Board).Asic_Num) {
              iVar3 = __aeabi_uidiv(uVar6,gAddress_interval);
              *(double *)((int)ext_data + (iVar3 + 3) * 8) =
                   (((double)(longlong)(int)(register_value & 0x7fff) / 2048.0) / 8.0 - 1.0) * 0.6;
              iVar3 = __aeabi_uidiv(uVar6,gAddress_interval);
              *(undefined1 *)((int)ext_data + iVar3 + 0x37c) = 1;
            }
          }
          ...
          if (which_function_local != 0xe) goto LAB_0002dd34;
          uVar2 = (uint)gPattern_test_counter;
          iVar3 = __aeabi_uidiv(reg_value_buf->reg_buffer[reg_value_buf->p_rd].chip_address,
                                gAddress_interval);
