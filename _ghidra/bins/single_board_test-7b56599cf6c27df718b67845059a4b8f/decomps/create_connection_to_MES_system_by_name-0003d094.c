...

{
  ...
  for (i = '\0'; (uint)i < (uint)get_local_ip_timeout_counter; i = i + '\x01') {
    ...
    if (iVar2 == 0) {
      print_time_stamp(time_stamp,0x30);
      printf("%s ",time_stamp);
      printf("%s : %s: %s\n","create_connection_to_MES_system_by_name",&DAT_001edb4c,ip);
      snprintf(tmp1,0x100,"%s: %s",&DAT_001edb4c,ip);
      log_to_file(tmp1,time_stamp);
      break;
    }
    if (get_local_ip_timeout_counter + -1 <= (int)(uint)i) {
      ...
      print_time_stamp(time_stamp,0x30);
      printf("%s ",time_stamp);
      printf("%s : Can\'t get %s ip\n","create_connection_to_MES_system_by_name",&DAT_001edb4c);
      snprintf(tmp1,0x100,"Can\'t get %s ip",&DAT_001edb4c);
      log_to_file(tmp1,time_stamp);
      return false;
    }
  }
