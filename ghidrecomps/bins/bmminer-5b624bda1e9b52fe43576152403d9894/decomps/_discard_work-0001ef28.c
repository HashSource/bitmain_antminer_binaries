...

{
  ...
  if (pwVar1 == (work *)0x0) {
    ...
    if (((pwVar1->clone == false) && (pwVar1->rolls == 0)) && (pwVar1->mined == false)) {
      ppVar2 = pwVar1->pool;
      if (ppVar2 != (pool *)0x0) {
        ppVar2->discarded_work = ppVar2->discarded_work + 1;
        ppVar2->quota_used = ppVar2->quota_used + -1;
        ppVar2->works = ppVar2->works + -1;
      }
      bVar3 = 0xfffffffe < (uint)total_discarded;
      total_discarded._0_4_ = (uint)total_discarded + 1;
      total_discarded._4_4_ = total_discarded._4_4_ + (uint)bVar3;
      if ((opt_debug != false) &&
