...

{
  ...
  if (dev->chain_exist[chain] == 0) {
    ...
    target_vol_00 = power_info[chain].working_voltage;
    ...
    if (3 < log_level) {
      ...
      if (pFVar2 != (FILE *)0x0) {
        fprintf(pFVar2,"%s:%d:%s: chain[%d] slowly setting to voltage: %0.2f ...\n","power.c",0x3d2,
                "slowly_set_iic_power_to_working_voltage_by_chain",(uint)chain,uVar3,uVar4);
      }
      fclose(pFVar2);
    }
    _Var1 = slowly_adapt_voltage(power_info + chain,target_vol_00,0);
    if (_Var1) {
      _Var1 = true;
    }
    else {
