...

{
  ...
  if (((res == (json_t *)0x0) || (res->type != JSON_TRUE)) &&
    ...
    *(uint *)((int)&pool_00->rejected + 4) = iVar6 + (uint)(0xfffffffe < uVar4);
    ...
    if ((opt_quiet != true) && (opt_realquiet != true)) {
      ...
      if (res_local == (json_t *)0x0) {
        if ((work->stratum != false) && (err != (json_t *)0x0)) {
          if ((err == (json_t *)0x0) || (err->type != JSON_ARRAY)) {
            if ((err != (json_t *)0x0) && (err->type == JSON_STRING)) {
              pcVar5 = json_string_value(err);
              snprintf(reason,0x1f," (%s)",pcVar5);
            }
          }
          else {
        ...
        pcVar5 = json_string_value(res_local);
        reasonLen = strlen(pcVar5);
        if (0x1c < reasonLen) {
          reasonLen = 0x1c;
        }
        reason[0] = ' ';
        reason[1] = '(';
        _cg_memcpy(reason + 2,pcVar5,reasonLen,"cgminer.c","share_result",0xe1e);
        reason[reasonLen + 2] = ')';
        reason[reasonLen + 3] = '\0';
        _cg_memcpy(disposition + 7,pcVar5,reasonLen,"cgminer.c","share_result",0xe23);
        disposition[6] = ':';
      ...
      if (((use_syslog != false) || (opt_log_output != false)) || (4 < opt_log_level)) {
        ...
        snprintf(tmp42,0x800,"Rejected %s %s %d %s%s %s%s",hashshow,pcVar1->drv->name,
                 pcVar1->device_id,where,reason,pcVar5,worktime);
    ...
    *(uint *)((int)&pool_00->accepted + 4) = iVar6 + (uint)(0xfffffffe < uVar4);
    ...
    if ((opt_quiet != true) && (opt_realquiet != true)) {
      if (total_pools < 2) {
        ...
        snprintf(tmp42,0x800,"Accepted %s %s %d pool %d %s%s",hashshow,pcVar1->drv->name,
                 pcVar1->device_id,work->pool->pool_no,pcVar5,worktime);
        _applog(5,tmp42,false);
      }
    }
