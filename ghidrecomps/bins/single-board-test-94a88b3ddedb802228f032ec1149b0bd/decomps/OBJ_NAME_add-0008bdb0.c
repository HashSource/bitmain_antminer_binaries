...

{
  ...
  if (names_lh == (_LHASH *)0x0) {
    CRYPTO_mem_ctrl(3);
    names_lh = lh_new((LHASH_HASH_FN_TYPE)0x8ba89,(LHASH_COMP_FN_TYPE)0x8ba49);
    CRYPTO_mem_ctrl(2);
    if (names_lh == (_LHASH *)0x0) {
      return 0;
    }
  }
  data_00 = (uint *)CRYPTO_malloc(0x10,"o_names.c",0xbf);
  if (data_00 == (uint *)0x0) {
    return 0;
  }
  *data_00 = type & 0xffff7fff;
  ...
  if (ptr == (int *)0x0) {
    iVar1 = 1 - names_lh->error;
    if (1 < (uint)names_lh->error) {
      iVar1 = 0;
    }
    return iVar1;
  }
