...
LAB_000c6e1a:
    ...
    if (title != (char *)0x0) {
      iVar1 = indent + 4;
      BIO_printf(out,"%*s%s:\n",indent,&DAT_0013a6fc,title);
    }
    iVar4 = 0;
    do {
      iVar2 = sk_num(&exts->stack);
      if (iVar2 <= iVar4) goto LAB_000c6e1a;
      ex = (X509_EXTENSION *)sk_value(&exts->stack,iVar4);
      if ((iVar1 != 0) && (iVar2 = BIO_printf(out,"%*s",iVar1,&DAT_0013a6fc), iVar2 < 1)) break;
      ...
      if (iVar2 == 0) {
        BIO_printf(out,"%*s",iVar1 + 4,&DAT_0013a6fc);
