...

{
  ...
  if (((0 < numasc) && (-1 < asc_local)) && (asc_local < numasc)) {
    dev = ascdevice(asc_local);
    if (-1 < dev) {
      cgpu = get_devices(dev);
      temp = (float)cgpu->temp;
      dev_runtime = cgpu_runtime(cgpu);
      cgpu->utility = ((double)(longlong)cgpu->accepted / dev_runtime) * 60.0;
      ...
      uVar2 = cgpu->hw_errors;
      uVar1 = (uint)cgpu->diff1;
      if (uVar2 + uVar1 == 0 &&
          ((int)uVar2 >> 0x1f) + *(int *)((int)&cgpu->diff1 + 4) + (uint)CARRY4(uVar2,uVar1) == 0) {
        hwp = 0.0;
      }
      else {
        iVar4 = cgpu->hw_errors;
        uVar2 = cgpu->hw_errors;
        uVar1 = (uint)cgpu->diff1;
        dVar3 = (double)__aeabi_l2d(uVar2 + uVar1,
                                    ((int)uVar2 >> 0x1f) +
                                    *(int *)((int)&cgpu->diff1 + 4) + (uint)CARRY4(uVar2,uVar1));
        hwp = (double)(longlong)iVar4 / dVar3;
      }
      root = api_add_percent(root,"Device Hardware%",&hwp,false);
      if ((int)cgpu->diff1 == 0 && *(int *)((int)&cgpu->diff1 + 4) == 0) {
        rejp = 0.0;
      }
      else {
        dVar5 = cgpu->diff_rejected;
        dVar3 = (double)__aeabi_l2d((int)cgpu->diff1,*(undefined4 *)((int)&cgpu->diff1 + 4));
        rejp = dVar5 / dVar3;
      }
