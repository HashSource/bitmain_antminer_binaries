...

{
  ...
  do {
    ...
    if ((sched_paused == true) || (_Var1 = should_run(), _Var1)) {
      if ((sched_paused != false) && (_Var1 = should_run(), _Var1)) {
        ...
        for (i = 0; i < mining_threads; i = i + 1) {
          ptVar4 = get_thread(i);
          if (ptVar4->cgpu->deven != DEV_DISABLED) {
            ptVar4->pause = false;
            if ((opt_debug != false) &&
               (((use_syslog != false || (opt_log_output != false)) || (6 < opt_log_level)))) {
              snprintf(tmp42,0x800,"Pushing sem post to thread %d",ptVar4->id);
              _applog(7,tmp42,false);
            }
            _cgsem_post(&ptVar4->sem,"cgminer.c","watchdog_thread",0x2851);
          }
        }
      }
    }
    ...
    for (i = 0; i < total_devices; i = i + 1) {
      ...
      ptVar4 = *cgpu_00->thr;
      if (ptVar4 != (thr_info *)0x0) {
        (*cgpu_00->drv->get_stats)(cgpu_00);
        iVar3 = cgpu_00->device_id;
        snprintf(dev_str,8,"%s %d",cgpu_00->drv->name,iVar3);
        if ((ptVar4->getwork == false) && (cgpu_00->deven != DEV_DISABLED)) {
          if ((cgpu_00->status == LIFE_WELL) || (0x77 < now.tv_sec - (ptVar4->last).tv_sec)) {
            if ((cgpu_00->status == LIFE_WELL) && (0x78 < now.tv_sec - (ptVar4->last).tv_sec)) {
              ...
              *(undefined4 *)((int)&cgpu_00->rolling + 4) = 0;
              ...
              if (opt_restart != false) {
                if (((use_syslog != false) || (opt_log_output != false)) || (2 < opt_log_level)) {
                  snprintf(tmp42,0x800,"%s: Attempting to restart",dev_str,iVar3);
                  _applog(3,tmp42,false);
                }
                reinit_device(cgpu_00);
              }
            }
            else if ((cgpu_00->status == LIFE_SICK) && (600 < now.tv_sec - (ptVar4->last).tv_sec)) {
            ...
            else if ((0x3c < now.tv_sec - (ptVar4->sick).tv_sec) &&
                    (((cgpu_00->status == LIFE_SICK || (cgpu_00->status == LIFE_DEAD)) &&
                     (cgtime(&ptVar4->sick), opt_restart != false)))) {
              reinit_device(cgpu_00);
            }
          }
          else {
            if ((cgpu_00->status != LIFE_INIT) &&
              ...
              snprintf(tmp42,0x800,"%s: Recovered, declaring WELL!",dev_str,iVar3);
              _applog(3,tmp42,false);
            }
