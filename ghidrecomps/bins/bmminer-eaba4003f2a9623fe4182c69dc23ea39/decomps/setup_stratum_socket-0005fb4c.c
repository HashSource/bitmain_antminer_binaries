      ...
      if (sockd == -1) {
        ...
        if (_Var2) {
          do {
            ...
            rw.fds_bits[iVar3 >> 5] = rw.fds_bits[iVar3 >> 5] | 1 << (sockd % 0x20 & 0xffU);
            ...
            if (0 < selret) {
              ...
              if ((rw.fds_bits[iVar3 >> 5] >> (sockd % 0x20 & 0xffU) & 1U) != 0) {
                len = 4;
...
LAB_0006010c:
    if (p == (addrinfo *)0x0) {
      ...
      if (pool->sockbuf == (char *)0x0) {
        pcVar4 = (char *)_cgcalloc(0x2000,1,"util.c","setup_stratum_socket",0xb6f);
        pool->sockbuf = pcVar4;
