  ...
  if (3 < log_level) {
    ...
    if (pFVar2 != (FILE *)0x0) {
      fprintf(pFVar2,"%s:%d:%s: down_freq_n_steps chain[%d] steps = %d\n","driver-btm-c5.c",0x2788,
              "down_freq_n_steps",param_1,param_3);
    }
    fclose(pFVar2);
  }
  uVar8 = 1;
...
LAB_00035610:
    ...
    pFVar2 = fopen(log_file,"a+");
    if (pFVar2 != (FILE *)0x0) {
      fprintf(pFVar2,"%s:%d:%s: max freq = %d\n","driver-btm-c5.c",0x27ad,"down_freq_n_steps",
    ...
    if (3 < log_level) {
      pFVar2 = fopen(log_file,"a+");
      if (pFVar2 != (FILE *)0x0) {
        fprintf(pFVar2,"%s:%d:%s: dev->timeout = %d\n","driver-btm-c5.c",0x27af,"down_freq_n_steps",
                *(undefined4 *)(dev + 0x48));
      }
      fclose(pFVar2);
      goto LAB_00035610;
...
LAB_00035620:
    ...
    if (config_parameter._18_2_ == 0) {
      uVar4 = calculate_core_number(*(undefined1 *)(dev + 0x542c));
      iVar6 = dev;
      ...
      *(uint *)(iVar6 + 0x48) = uVar8;
      ...
      uVar8 = (uint)config_parameter[0x12] * 1000 + (uint)config_parameter[0x13];
      *(uint *)(dev + 0x48) = uVar8;
    }
    if (0x1ffff < uVar8) {
  ...
  if (3 < log_level) {
    pFVar2 = fopen(log_file,"a+");
    if (pFVar2 != (FILE *)0x0) {
      fprintf(pFVar2,"%s:%d:%s: dev->timeout = %d\n","driver-btm-c5.c",0x27c1,"down_freq_n_steps",
              *(undefined4 *)(dev + 0x48));
    }
    fclose(pFVar2);
  }
