...

{
    ...
    if (3 < uVar1) {
      ...
      if (pFVar3 != (FILE *)0x0) {
        fprintf(pFVar3,"%s:%d:%s: Chain[J%d] try read temp chip[%d] ID=%02x (I2C addr=0x98)\n",
                "driver-btm-c5.c",0x18c4,"do_calibration_sensor_offset",param_3 + 1,param_2,
      ...
      if (3 < uVar1) {
        ...
        if (pFVar3 != (FILE *)0x0) {
          fprintf(pFVar3,"%s:%d:%s: Chain[J%d] try read temp chip[%d] ID=%02x (I2C addr=0x9A)\n",
                  "driver-btm-c5.c",0x18d0,"do_calibration_sensor_offset",param_3 + 1,param_2,
                  (uint)*(byte *)(dev + iVar5 + param_4 + 0x468));
        }
        fclose(pFVar3);
        cVar2 = *(char *)(dev + iVar5 + param_4 + 0x468);
        ...
        if (3 < uVar1) {
          ...
          if (pFVar3 != (FILE *)0x0) {
            fprintf(pFVar3,"%s:%d:%s: Chain[J%d] try read temp chip[%d] ID=%02x (I2C addr=0x9C)\n",
                    "driver-btm-c5.c",0x18dc,"do_calibration_sensor_offset",param_3 + 1,param_2,
                    (uint)*(byte *)(dev + iVar5 + param_4 + 0x468));
          }
          fclose(pFVar3);
  ...
  local_2c = (char)(int)(0.0 - (float)(((double)(longlong)iVar6 -
                                       ((double)(longlong)iVar4 + 273.15) * 0.10119047619047628) -
    ...
    if (log_level < 4) {
      ...
      if (pFVar3 != (FILE *)0x0) {
        fprintf(pFVar3,
                ...
                ,"driver-btm-c5.c",0x18fe,"do_calibration_sensor_offset",param_3,param_2,iVar4,iVar6
                ,iVar5);
      }
      fclose(pFVar3);
      iVar5 = 0x1b;
  ...
  if (3 < log_level) {
    ...
    if (pFVar3 != (FILE *)0x0) {
      fprintf(pFVar3,"%s:%d:%s: New offset Chain[%d] chip[%d] local:%hhd remote:%hhd offset:%hhd \n"
              ,"driver-btm-c5.c",0x1905,"do_calibration_sensor_offset",param_3 + 1,param_2,iVar4,
              iVar6,iVar5);
    }
    fclose(pFVar3);
    return iVar5;
